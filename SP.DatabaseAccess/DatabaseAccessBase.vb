...
#Region "Methods"
	...
	Protected Function OpenReader(ByVal sql As String, ByVal parameters As IEnumerable(Of SqlParameter), Optional ByVal commandType As System.Data.CommandType = CommandType.Text) As SqlClient.SqlDataReader
			...
			If Not parameters Is Nothing Then
				For Each param In parameters
					...
					If param.Direction = ParameterDirection.Output Then
						paramData.Add(String.Format("{0} {1} ({2}) = {3}", param.ToString(), param.DbType, "(OUTPUT)", param.Value))
						'Trace.WriteLine(String.Format("{0} {1} ({2}) = {3}", param.ToString(), param.DbType, "(OUTPUT)", param.Value))
					Else
						paramData.Add(String.Format("{0} {1} ({2}) = {3}", param.ToString(), param.DbType, If(param.DbType = DbType.String, Len(param.Value.ToString()), 0), param.Value))
						'Trace.WriteLine(String.Format("{0} {1} ({2}) = {3}", param.ToString(), param.DbType, If(param.DbType = DbType.String, Len(param.Value.ToString()), 0), param.Value))
					End If
		...
		Catch e As Exception
			m_Logger.LogError(String.Format("Constring={1}{0}SQL={2}{0}Parameter={3}{0}Exception={4}", vbNewLine, DBConnectionstring, sql, String.Join(vbNewLine, paramData), e.ToString()))
