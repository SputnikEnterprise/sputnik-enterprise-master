...
#Region "Private Consts"
  ...
  Private Const CHURCH_TAX_CODE_PLUS = "+"
...
#Region "Public Methods"
  ...
  Public Sub DetermineQSTCodeData(
    ...
    newQSTCode = currentQSTCode
		...
		Else
			...
			For Each c In codes
				Dim cviewData = New QSTCodeViewData With {.Code = c,
																									.Translation = taxHelper.GetTranslationForCode(c)}
  ...
  Public Sub DetermineNumberOfChildrenData(ByVal qstCode As String,
    ...
    If permission = PERMISSION_CODE_C Or
      permission = PERMISSION_CODE_S Or
      taxHelper Is Nothing Then
...
#Region "Private Methods"
  ...
  ''' </summary>
  Private Function CreateNoQSTViewDataObject(ByVal qstTranslationData As IEnumerable(Of QSTData)) As QSTCodeViewData
    Return New QSTCodeViewData With {.Code = Code_NoQST,
                                    .Translation = qstTranslationData.Where(Function(data) data.GetField = Code_NoQST).Select(Function(data) data.TranslatedQSTCodeText).FirstOrDefault()
                                    }
  End Function
