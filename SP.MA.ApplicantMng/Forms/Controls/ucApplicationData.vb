...
#Region "readonly properties"
		...
		Private ReadOnly Property SelectedRecord As ApplicationData
			Get
				Dim gv = TryCast(grdExistsMSalary.MainView, DevExpress.XtraGrid.Views.Grid.GridView)
...
#Region "Private Methods"
		...
		Private Sub ResetMonthlySalaryOverviewGrid()
			...
			gvExistsMSalary.OptionsView.ShowColumnHeaders = True
			...
			columnCreatedOn.Caption = m_Translate.GetSafeTranslationValue("Eingang")
			columnCreatedOn.Name = "CreatedOn"
			columnCreatedOn.FieldName = "CreatedOn"
			columnCreatedOn.Visible = True
			columnCreatedOn.Width = 70
			gvExistsMSalary.Columns.Add(columnCreatedOn)


			grdExistsMSalary.DataSource = Nothing
		...
		Private Function LoadMonthlySalaryList(ByVal employeeNumber As Integer, ByVal onlyActiveRecords As Boolean) As Boolean
			Dim salaryOverviewList = m_EmployeeDataAccess.LoadAssignedEmployeeApplications(CustomerID, employeeNumber, 0, onlyActiveRecords)
			...
			grdExistsMSalary.DataSource = listDataSource
		...
		Private Sub OnMonthlySalary_DoubleClick(sender As System.Object, e As System.EventArgs) Handles gvExistsMSalary.DoubleClick
			...
			If (selectedRows.Count > 0) Then
				Dim monthlySalaryData = CType(gvExistsMSalary.GetRow(selectedRows(0)), ApplicationData)
				ShowMonthlySalaryDetailForm(m_EmployeeNumber, monthlySalaryData.ID)
			End If
		End Sub
