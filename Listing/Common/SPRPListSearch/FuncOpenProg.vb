...
#Region "Funktionen für Exportieren..."
	...
	Sub RunSMSProg()
		...
		If Not File.Exists(strSMSFile) Then
			Dim strMessage As String = "Folgende Datei wurde nicht gefunden. Bitte wählen Sie das Programm aus.{0}"
			strMessage = m_Translate.GetSafeTranslationValue(String.Format(strMessage, vbNewLine))
			MsgBox(strMessage &
							(strSMSFile), MsgBoxStyle.Critical, m_Translate.GetSafeTranslationValue("Programm wurde nicht gefunden"))
	...
	Sub ExportDataToOutlook(ByVal strTempSQL As String)
		...
		Try
			If MsgBox(m_Translate.GetSafeTranslationValue("Dieser Vorgang kann mehrer Minuten dauern. Sind Sie sicher?"),
								MsgBoxStyle.Information + MsgBoxStyle.YesNo, m_Translate.GetSafeTranslationValue("Daten exportieren")) = MsgBoxResult.Yes Then
				oMyProg = CreateObject("SPSModulsView.ClsMain")
