...
#End Region
	...
	Function LoadMandantenData() As IEnumerable(Of MandantenData)
		...
		Dim reader As SqlClient.SqlDataReader = m_utility.OpenReader(m_InitializationData.MDData.MDDbConn, sql, Nothing, CommandType.StoredProcedure)
	...
	Function LoadEmployeeData(ByVal firstMonth As Integer?, ByVal firstYear As Integer?) As IEnumerable(Of EmployeeData)
		...
		sql &= "Left Join RP On MA.MANr = RP.MANr "
		sql &= "Where RP.Erfasst <> 1 "
		...
		listOfParams.Add(New SqlClient.SqlParameter("MDNr", m_utility.ReplaceMissing(m_InitializationData.MDData.MDNr, 0)))
		...
		Dim reader As SqlClient.SqlDataReader = m_utility.OpenReader(m_InitializationData.MDData.MDDbConn, sql, listOfParams, CommandType.Text)
	...
	Function LoadCustomerData(ByVal firstMonth As Integer?, ByVal firstYear As Integer?, ByVal employeeNumber As Integer?) As IEnumerable(Of CustomerData)
		...
		sql &= "FROM Kunden KD "
		sql &= "Left Join RP On KD.KDNr = RP.KDNr "
		sql &= "Where RP.Erfasst <> 1 "
		sql &= "And RP.MDNr = @MDNr "
		sql &= "And RP.Monat = @firstMonth "
		sql &= "And RP.Jahr = @firstYear "
...
#Region "Funktionen zur Suche nach Daten..."
	...
	Function GetQuerySQLString(ByVal sSQLQuery As String, ByVal searchData As SearchCriteria) As String
		...
		If m_InitializationData.MDData.MultiMD = 1 Then
			strAndString = IIf(sSql <> String.Empty, " And ", String.Empty).ToString
			sSql += String.Format("{0}RP.MDNr = {1}", strAndString, m_InitializationData.MDData.MDNr)
		End If
	...
	Function GetStartSQLString_2() As String
		...
		sSql += "KD.Firma1, KD.Ort As KSOrt, ES.ES_Ab, ES.ES_Ende, ES.ES_Als, "
		...
		sSql += CStr(m_InitializationData.UserData.UserNr)
	...
	Sub GetJobNr4Print(ByVal frmSource As frmRPNotDoneSearch)
		Dim strModul2print As String = String.Empty

		With frmSource
			strModul2print = "10.8"  ' Druck für Rapportlisten
...
#Region "Sonstige Funktionen für Datenbank..."
	...
	Function BuildRPDayDb(ByVal strQuery As String) As Boolean
		...
		Try
			...
			param = cmd.Parameters.AddWithValue("@USNr", m_InitializationData.UserData.UserNr)
	...
	Sub GetInsertString(ByVal iRPNr As Integer, ByVal dRPVon As Date, ByVal dRPBis As Date)
		...
		Try
			...
			param = cmd.Parameters.AddWithValue("@USNr", CInt(m_InitializationData.UserData.UserNr))
