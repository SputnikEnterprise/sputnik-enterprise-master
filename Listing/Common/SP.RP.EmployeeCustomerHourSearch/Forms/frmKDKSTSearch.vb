...
#Region "private fields"
		...
		Private m_SearchCalculationEnum As CalculationTypeEnum
...
#Region "Constructor"
		...
		Public Sub New(ByVal _setting As SP.Infrastructure.Initialization.InitializeClass)
			...
			xtabCtlMain.TabPages.Remove(xtabPrintView)
			TranslateControls()
			Reset()
			...
			AddHandler luePVL.ButtonClick, AddressOf OnDropDown_ButtonClick
...
#Region "employment"
		...
		Private Sub ResetCustomerGrid()
			...
			Dim columnCostcenterName As New DevExpress.XtraGrid.Columns.GridColumn()
			columnCostcenterName.OptionsFilter.AutoFilterCondition = DevExpress.XtraGrid.Columns.AutoFilterCondition.Contains
			columnCostcenterName.OptionsColumn.AllowEdit = False
			columnCostcenterName.Caption = m_Translate.GetSafeTranslationValue("Kostenstelle")
			columnCostcenterName.Name = "CostcenterName"
			columnCostcenterName.FieldName = "CostcenterName"
			columnCostcenterName.Visible = True
			columnCostcenterName.Width = 80
			gvPrint.Columns.Add(columnCostcenterName)
			...
			columnAmount.Caption = m_Translate.GetSafeTranslationValue("Total Betrag")
			...
			gvPrint.Columns.Add(columnAmount)
...
#Region "loading customer dropdown data"
...
#Region "loading employee dropdown data"
...
#End Region
		...
		Sub TranslateControls()
			...
			Me.bbiWithDetails.Caption = m_Translate.GetSafeTranslationValue(Me.bbiWithDetails.Caption)
...
#End Region
		...
		Private Function LoadSearchKonditions() As Boolean
			...
			m_SearchCalculationEnum = If(sbHourLA.Value, CalculationTypeEnum.HourCalculation, CalculationTypeEnum.AllCalculation)
			...
			searchData.CalculationType = m_SearchCalculationEnum
		...
		Private Sub LoadCurstomerHourGrid()
			...
			Dim gridData = (From person In foundedData
											Select New CustomertReportHoursData With {.MDNr = person.MDNr,
																															...
																															.CustomerCostcenter = person.CustomerCostcenter,
																															.LANr = person.LANr,
																															.RPKST = person.RPKST,
																															.Company1 = person.Company1,
																															.Company2 = person.Company2,
																															.Company3 = person.Company3,
																															.CostcenterName = person.CostcenterName,
		...
		Private Function LoadCustomerSearchResultData(ByVal searchKind As HourSearchTypeEnum) As IEnumerable(Of CustomertReportHoursData)
			Dim data = m_ListingDatabaseAccess.SearchCustomerHoursReportlineEachCostcenterData(lueMandant.EditValue, m_SearchKindEnum, m_InitializationData.UserData.UserFiliale, m_SearchCriteriums)
		...
		Private Sub OngvPrint_MasterRowGetChildList(ByVal sender As Object, ByVal e As MasterRowGetChildListEventArgs) Handles gvPrint.MasterRowGetChildList
			...
			If m_SearchKindEnum = HourSearchTypeEnum.Customer Then
				...
				listOfInvoiceData = LoadCustomerReportLineData(customerData)
				'listOfInvoiceData = m_ListingDatabaseAccess.SearchAssignedCustomerHoursReportlineEachCostcenterData(lueMandant.EditValue, customerData.CustomerNumber, customerData.CustomerCostcenter, customerData.LANr, m_InitializationData.UserData.UserFiliale, m_SearchCriteriums)

			ElseIf m_SearchKindEnum = HourSearchTypeEnum.Employee Then
				Dim employeeData = SelectedEmployeeRecord
				listOfInvoiceData = LoadEmployeeReportLineData(employeeData)
		...
		Private Function LoadCustomerReportLineData(ByVal customerData As CustomertReportHoursData) As IEnumerable(Of ReportlineHoursData)
			Dim result = New List(Of ReportlineHoursData)

			If customerData Is Nothing Then Return Nothing
			result = m_ListingDatabaseAccess.SearchAssignedCustomerHoursReportlineEachCostcenterData(lueMandant.EditValue, customerData.CustomerNumber, customerData.CustomerCostcenter,
																																															 customerData.RPKST, customerData.LANr, m_InitializationData.UserData.UserFiliale, m_SearchCriteriums)
		...
		Private Function LoadEmployeeReportLineData(ByVal employeeData As EmployeeReportHoursData) As IEnumerable(Of ReportlineHoursData)
			Dim result = New List(Of ReportlineHoursData)

			If m_SearchKindEnum = HourSearchTypeEnum.Customer Then
				Dim customerData = SelectedCustomerRecord
				result = m_ListingDatabaseAccess.SearchAssignedCustomerHoursReportlineEachCostcenterData(lueMandant.EditValue, customerData.CustomerNumber, customerData.CustomerCostcenter,
																																																 customerData.CustomerCostcenter, customerData.LANr, m_InitializationData.UserData.UserFiliale, m_SearchCriteriums)

				If employeeData Is Nothing Then Return Nothing
				result = m_ListingDatabaseAccess.SearchAssignedEmployeeHoursReportlineData(lueMandant.EditValue, employeeData.Employeenumber, m_InitializationData.UserData.UserFiliale, m_SearchCriteriums)
...
#End Region
		...
		Private Sub bbiPrint_ItemClick(sender As Object, e As ItemClickEventArgs) Handles bbiPrint.ItemClick
			...
			'If gvPrint.RowCount > 0 Then
			'	' Opens the Preview window. 
			'	gvPrint.ShowPrintPreview()
			'End If

			Dim CustomerLinesData As BindingList(Of CustomertReportHoursData) = grdPrint.DataSource
			Dim ReportLinesToPrintData As BindingList(Of ReportlineHoursData) = Nothing

			Dim CustomerPrintData As New List(Of CustomerReportLinesPrintData)
			For Each itm In CustomerLinesData
				Dim viewData = New CustomerReportLinesPrintData

				viewData.MDNr = itm.MDNr
				viewData.CustomerNumber = itm.CustomerNumber
				viewData.CustomerCostcenter = itm.CustomerCostcenter
				viewData.LANr = itm.LANr
				viewData.Company1 = itm.Company1
				viewData.Company2 = itm.Company2
				viewData.Company3 = itm.Company3
				viewData.CostcenterName = itm.CostcenterName
				viewData.Street = itm.Street
				viewData.Postcode = itm.Postcode
				viewData.PostOfficeBox = itm.PostOfficeBox
				viewData.Location = itm.Location
				viewData.CountryCode = itm.CountryCode
				viewData.EMail = itm.EMail
				viewData.Telefax = itm.Telefax
				viewData.Telephone = itm.Telephone
				viewData.TotalHours = itm.TotalHours
				viewData.Amount = itm.Amount
				viewData.FirstProperty = itm.FirstProperty

				viewData.ReportLineData = LoadCustomerReportLineData(itm)


				CustomerPrintData.Add(viewData)

			Next

			'Dim Report = New DevExpress.XtraReports.UI.XtraReport()
			If bbiWithDetails.Checked Then
				Dim Report = New XtraReport1()
				Report.InitialData = m_InitializationData
				Report.CustomerLinesToPrintData = CustomerPrintData
				Report.m_ListingDatabaseAccess = m_ListingDatabaseAccess
				Dim result As Boolean = Report.LoadData()

			Else
				Dim Report = New reportKstDataGrouped()
				Report.InitialData = m_InitializationData
				Report.CustomerLinesToPrintData = CustomerPrintData
				Report.m_ListingDatabaseAccess = m_ListingDatabaseAccess
				Dim result As Boolean = Report.LoadData()
			...
			'Dim tplFileName As String = Path.Combine(m_InitializationData.MDData.MDDocumentPath, "KstStdData.repx")
			'Report.SaveLayout(tplFileName)
			'Report.LoadLayout(tplFileName)
			...
			' show designer
			'Dim designTool = New ReportDesignTool(Report)
			'designTool.ShowDesignerDialog()

			'Tool.ShowPageSetup()

			'PrintWithPageInformation()
			'StartPrinting()

		End Sub

		Private Sub PrintWithPageInformation()
			...
			'PrintableComponentLink1.Component = grdPrint
			...
			xtabCtlMain.SelectedTabPage = xtabPrintView
		...
		Private Sub StartPrinting()
			...
			Dim _Setting As New SPS.Listing.Print.Utility.ReportPrint.HoursPrintData With {.ShowAsDesign = showDesign,
																																									 ...
																																									 .PrintJobNumber = If(m_SearchKindEnum = HourSearchTypeEnum.Customer, "10.0.2", "1.3.2")}
			If m_SearchKindEnum = HourSearchTypeEnum.Customer Then
				_Setting.CustomerHoursToPrintData = grdPrint.DataSource
			Else
				_Setting.EmployeeHoursToPrintData = grdPrint.DataSource
			End If
			Dim obj As New SPS.Listing.Print.Utility.ReportPrint.CLSPrintingReportHours_KDKST(m_InitializationData)
...
#Region "bbExport"
		...
		Private Sub bbiWithDetails_CheckedChanged(sender As Object, e As ItemClickEventArgs) Handles bbiWithDetails.CheckedChanged
