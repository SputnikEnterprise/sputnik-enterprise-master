...
#End Region
	...
	Function GetSearchKrieteria() As SearchCriteria
		...
		result.listname = m_Translate.GetSafeTranslationValue("Kunden-Kostenstellen Liste")
		result.mandantenname = lueMandant.Text

		result.KDNrList = cbo_KDNr.EditValue
		result.ESNrList = cbo_ESNr.EditValue
		result.MANrList = cbo_MANr.EditValue
		...
		Dim lastmonth As Integer = If(Cbo_BMonth_1.EditValue Is Nothing, Cbo_VMonth_1.EditValue, Cbo_BMonth_1.EditValue)
...
#End Region
	...
	Sub StartPrinting()
		...
		strFilter &= If(Not m_SearchCriteria.MANrList Is Nothing, String.Format("{1}Kunden: {0}", m_SearchCriteria.MANrList, vbNewLine), String.Empty)
		strFilter &= If(Not m_SearchCriteria.MANrList Is Nothing, String.Format("{1}Einsätze: {0}", m_SearchCriteria.MANrList, vbNewLine), String.Empty)
		...
		strFilter &= If(m_SearchCriteria.beruf.Length > 0, String.Format("{1}GAV-Beruf: {0}", m_SearchCriteria.beruf, vbNewLine), String.Empty)
	...
	Private Sub Cbo_FARListeBeruf_QueryPopUp(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles Cbo_FARListeBeruf.QueryPopUp
		ListFARListeGAVBeruf(Me.Cbo_FARListeBeruf, Me.Cbo_JahrVon.EditValue, Cbo_VMonth_1.EditValue, Cbo_BMonth_1.EditValue)
	End Sub

	''' <summary>
	''' Selektionsfenster für die erste Kunden-Nummer öffnen.
	''' </summary>
	Private Sub Oncbo_KDNr_ButtonClick(sender As Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles cbo_KDNr.ButtonClick
		Dim strMethodeName As String = New System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name
		...
		Me.cbo_KDNr.Text = If(m = Nothing, String.Empty, CStr(m.ToString.Replace(strValueSeprator, ",")))
		frmTest.Dispose()

	End Sub

	''' <summary>
	''' Selektionsfenster für die erste Kandidaten-Nummer öffnen.
	''' </summary>
	Private Sub Oncob_MANr_ButtonClick(sender As Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles cbo_MANr.ButtonClick
		Dim strMethodeName As String = New System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name
	...
	''' <summary>
	''' Selektionsfenster für die erste Einsatz-Nummer öffnen.
	''' </summary>
	Private Sub Oncob_ESNr_ButtonClick(sender As Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles cbo_ESNr.ButtonClick
		Dim strMethodeName As String = New System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name
		Dim frmTest As New frmSearchRec("ESNr")
