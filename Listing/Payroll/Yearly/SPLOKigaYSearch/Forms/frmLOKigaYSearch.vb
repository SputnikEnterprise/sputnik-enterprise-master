...
#End Region
	...
	'  Private _NotifyLVMoveDelegate As frmLOKigaYSearch_LV.NotifyLVMoveDel
	'#End Region
	...
	'    _ClsErrException.MessageBoxShowError(_ClsProgSetting.GetLogedUSNr, "NotifyMainProgressBar", ex)
	'  End Try
	...
	'    StatusStrip1.Update()
	'    Me.txt_SQLQuery.Text = ClsDataDetail.SQLQuery
	'  Catch ex As Exception ' Manager
	'    _ClsErrException.MessageBoxShowError(_ClsProgSetting.GetLogedUSNr, "DelegateProgress", ex)
	'  End Try
	...
	'    ' Das Delegate wurde nicht erstellt --> keine Fehlermeldung
	'  Catch ex As Exception ' Manager
	'    _ClsErrException.MessageBoxShowError(_ClsProgSetting.GetLogedUSNr, "NotifyStartLV", ex)
	'  End Try
	'End Sub
	...
	'    LblState_1.Text = String.Format("{0} Datensätze angezeigt", frmMyLV.LvFoundedrecs.Items.Count)
	...
	'  Catch ex As Exception ' Manager
	'    _ClsErrException.MessageBoxShowError(_ClsProgSetting.GetLogedUSNr, "DelegateLVCompleted", ex)
	'  End Try
	'End Sub
...
#End Region
	...
	'  Catch ex As Exception ' Manager
	'    _ClsErrException.MessageBoxShowError(_ClsProgSetting.GetLogedUSNr, "Get4Print", ex)
	'  Finally
	'    cmd.Dispose()
	'    Conn.Close()
	'    Me.StatusStrip1.Items("lblWaitpic").Visible = False
	...
	'      frmMyLV = New frmLOKigaYSearch_LV(Me.Location.X, Me.Location.Y, Me.Height, Me, _
	'                        New frmLOKigaYSearch_LV.NotifyMainProgressBarDel(AddressOf DelegateProgress), _
	'                        New frmLOKigaYSearch_LV.NotifyMainLVCompletedDel(AddressOf DelegateLVCompleted), _
	'                        New frmLOKigaYSearch_LV.NotifyMainAllowAbortDel(AddressOf DelegateAllowAbort))
	...
	Function GetSearchKrieteria() As SearchCriteria
		...
		result.listname = m_Translate.GetSafeTranslationValue("Kiga Statistiken")
	...
	Function GetMyQueryString() As Boolean
		...
		Try
			...
			'	'ACHTUNG: Wenn der Control txt_SQLQuery aufgebaut wird (einfach im Reiter SQL-Abfrage klicken),
			'	'         so gibt es ein Thread-Übergriff-Exception. Darum via Shared Variable übermitteln.
			'	'Me.txt_SQLQuery.Text = _ClsDb.GetQuerySQLString(frm)
			'	_ClsDb.GetQuerySQLString(frm)
			'Else
			'	ClsDataDetail.SQLQuery = Me.txt_IndSQLQuery.Text
			'End If
...
#End Region
	...
	Sub StartExportModul()
		Dim _Setting As New SPS.Export.Listing.Utility.ClsCSVSettings With {.DbConnString2Open = ClsDataDetail.m_InitialData.MDData.MDDbConn, _
																																			 ...
																																			 .ModulName = "KigaListToCSV"}
		Dim obj As New SPS.Export.Listing.Utility.ClsExportStart(_Setting)

		obj.ExportCSVFromKigaListing(Me.txt_SQLQuery.Text)
	...
	Sub StartPrinting()
		...
		strFilter &= If(String.IsNullOrWhiteSpace(m_SearchCriteria.filiale), String.Empty, String.Format("Filiale: {0}{1}", m_SearchCriteria.filiale, vbNewLine))

		Me.PrintJobNr = GetJobID()
		Me.SQL4Print = Me.txt_SQLQuery.Text

		Dim _Setting As New ClsLLKigaSearchPrintSetting With {.DbConnString2Open = m_InitializationData.MDData.MDDbConn,
		...
		Dim obj As New KigaSearchListing.ClsPrintKigaSearchList(_Setting)

		obj.PrintkigaSearchList()
