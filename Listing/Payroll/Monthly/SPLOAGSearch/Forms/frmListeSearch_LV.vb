...
Public Class frmListeSearch_LV
	...
	Private m_xmlSettingRestoreLoAGSearchSetting As String
	Private m_xmlSettingLoAGSearchFilter As String
...
#Region "Private Consts"
	...
	Private Const MODUL_NAME_SETTING = "loagsearchlist"

	Private Const USER_XML_SETTING_SPUTNIK_LOAG_SEARCH_LIST_GRIDSETTING_RESTORE As String = "gridsetting/User_{0}/loagsearchlist/{1}/restorelayoutfromxml"
	Private Const USER_XML_SETTING_SPUTNIK_LOAG_SEARCH_LIST_GRIDSETTING_FILTER As String = "gridsetting/User_{0}/loagsearchlist/{1}/keepfilter"

	Private Const USER_XML_SETTING_SPUTNIK_LOAGREKAP_SEARCH_LIST_GRIDSETTING_RESTORE As String = "gridsetting/User_{0}/loagrekapsearchlist/{1}/restorelayoutfromxml"
	Private Const USER_XML_SETTING_SPUTNIK_LOAGREKAP_SEARCH_LIST_GRIDSETTING_FILTER As String = "gridsetting/User_{0}/loagrekapsearchlist/{1}/keepfilter"
...
#Region "Constructor"
	...
	Public Sub New(ByVal strQuery As String, ByVal _modulname As String)
		...
		Try
			If Modul2Open = "loag" Then
				m_GridSettingPath = String.Format("{0}LOAGSearchList\", m_md.GetGridSettingPath(ClsDataDetail.m_InitialData.MDData.MDNr))
			ElseIf Modul2Open = "loagrekap" Then
				m_GridSettingPath = String.Format("{0}LOAGRekapSearchList\", m_md.GetGridSettingPath(ClsDataDetail.m_InitialData.MDData.MDNr))
			End If
			...
			m_xmlSettingRestoreLoAGSearchSetting = String.Format(USER_XML_SETTING_SPUTNIK_LOAG_SEARCH_LIST_GRIDSETTING_RESTORE, ClsDataDetail.m_InitialData.UserData.UserNr, MODUL_NAME_SETTING)
			m_xmlSettingLoAGSearchFilter = String.Format(USER_XML_SETTING_SPUTNIK_LOAG_SEARCH_LIST_GRIDSETTING_FILTER, ClsDataDetail.m_InitialData.UserData.UserNr, MODUL_NAME_SETTING)

			m_xmlSettingRestoreLoRekapSearchSetting = String.Format(USER_XML_SETTING_SPUTNIK_LOAGREKAP_SEARCH_LIST_GRIDSETTING_RESTORE, ClsDataDetail.m_InitialData.UserData.UserNr, MODUL_NAME_SETTING)
			m_xmlSettingLoRekapSearchFilter = String.Format(USER_XML_SETTING_SPUTNIK_LOAGREKAP_SEARCH_LIST_GRIDSETTING_FILTER, ClsDataDetail.m_InitialData.UserData.UserNr, MODUL_NAME_SETTING)
...
#End Region
	...
	Function GetDbSalaryData4Show() As IEnumerable(Of FoundedData)
		...
		Try
			' SELECT LOL.MANR, LOL.Jahr, LOL.LP, LOL.LANR, LA.LALOText, LOL.M_Bas, LOL.M_Anz, LOL.M_Ans, LOL.M_Btr, LOL.GAV_Kanton, LOL.GAV_Beruf, LOL.GAV_Gruppe1, Mitarbeiter.Nachname, Mitarbeiter.Vorname, Mitarbeiter.AHV_Nr, Mitarbeiter.AHV_Nr_New, Mitarbeiter.Land As MALand, Mitarbeiter.PLZ, Mitarbeiter.Ort, Mitarbeiter.Strasse, Mitarbeiter.GebDat, Mitarbeiter.KST As MAKST, 2014 As VonJahr, 1 As VonMonat, 2014 As BisJahr, 1 As BisMonat
	...
	Function GetDbLORekapData4Show() As IEnumerable(Of FoundedLORekapData)
		...
		sql = String.Format("Select * From _LORekap_{0} Order By LANr", ClsDataDetail.m_InitialData.UserData.UserNr)
...
#End Region
	...
	Sub Ongv_RowCellClick(sender As System.Object, e As DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs)
		...
		If (e.Clicks = 2) Then
			...
			Dim viewData As FoundedData
			Dim column = e.Column
			Dim dataRow = gvRP.GetRow(e.RowHandle)
			If Not dataRow Is Nothing Then

				If Modul2Open = "loag" Then
					viewData = CType(dataRow, FoundedData)
				Else
					Return
	...
	Private Sub RestoreGridLayoutFromXml()
		...
		Try
			restoreLayout = m_utility.ParseToBoolean(UserGridSettingsXml.GetSettingByKey(m_xmlSettingRestoreLoAGSearchSetting, False), True)
			keepFilter = m_utility.ParseToBoolean(UserGridSettingsXml.GetSettingByKey(m_xmlSettingLoAGSearchFilter, False), False)
