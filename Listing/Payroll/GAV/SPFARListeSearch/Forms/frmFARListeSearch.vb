...
Public Class frmFARListeSearch
	Inherits XtraForm
...
#End Region
	...
	Private ReadOnly Property GetJobID() As String
		Get
			...
			If String.IsNullOrWhiteSpace(m_SearchCriteria.beruf) Then
				Return "9.8.1" ' FAR-Inkasso-Stelle (Standard)
			Else
				If m_SearchCriteria.beruf.StartsWith("Gerüstbau") Or m_SearchCriteria.beruf.StartsWith("Marmor") Then
					Return "9.8.2" ' InkassoPool
				Else
					Return "9.8.1" ' FAR-Inkasso-Stelle (Standard)
				End If
			End If
...
#Region "Private Methods"
	...
	Private Sub TranslateControls()
		...
		Chk_Resor.Text = m_Translate.GetSafeTranslationValue(Chk_Resor.Text)
		...
		Dim strValue = "<b>230001</b> Location de services Parcs et jardins, pépinières et arboriculture GE (GE)<br>"
		strValue &= "<b>305001</b> Personalverleih Gebäudetechnik Schweiz (TI)<br>"
		strValue &= "<b>350001</b> Personalverleih Decken- und Innenausbausysteme Schweiz (BS)<br>"
		strValue &= "<b>355003</b> Personalverleih Malergewerbe BL (BL)<br>"
		strValue &= "<b>355004</b> Personalverleih Gipsergewerbe BL (BL)<br>"
		strValue &= "<b>355005</b> Personalverleih Gipsergewerbe BS (BS)<br>"
		strValue &= "<b>355006</b> Prestito di personale Gessatori, stuccatori, montatori a secco, plafonatori e intonacatori (TI)<br>"
		strValue &= "<b>365001</b> Personalverleih Hafner- und Plattenlegergewerbe Schweiz (FR, VD, GE, BL, TI)<br>"
		strValue &= "<b>365003</b> Personalverleih Plattenlegergewerbe BS und BL (BS)<br>"
		strValue &= "<b>365005</b> Prestito di personale Posa pavimenti TI (TI)<br>"
		strValue &= "<b>365006</b> Prestito di personale Piastrelle e mosaici TI<br>"
		strValue &= "<b>385001</b> Personalverleih Möbelindustrie Schweiz (JU, NE, VS, GE)<br>"
		strValue &= "<b>445001</b> Personalverleih Bauwerk Parkett AG, St. Margrethen (FR, JU, VS, VD, GE, BL)<br>"
		strValue &= "<b>455001</b> Personalverleih Basler Ausbaugewerbe<br>"
		strValue &= "<b>455002</b> Personalverleih Ausbaugewerbe Westschweiz (FR, JU, NE, VS, VD, GE, BL, BS, TI)"

		Me.lbLstlInfo.Text = String.Format("<size=11><b>" & m_Translate.GetSafeTranslationValue("Info") & "</b><size=8.25></br>" &
																	 m_Translate.GetSafeTranslationValue("In bestimmten Kantonen, für folgende GAV-Berufe werden <b>RESOR-Beitrage</b> bezogen:</br>{0}"),
																	 strValue)
...
#End Region
	...
	Function GetSearchKrieteria() As SearchCriteria
		...
		result.listname = m_Translate.GetSafeTranslationValue("FAR-Lohnbescheinigung")
		...
		result.resor = Chk_Resor.Checked
...
#End Region
	...
	Sub StartExportModul()
		Dim _Setting As New SPS.Export.Listing.Utility.ClsCSVSettings With {.DbConnString2Open = ClsDataDetail.m_InitialData.MDData.MDDbConn, _
																																			 ...
																																			 .ModulName = "FARListToCSV"}
		Dim obj As New SPS.Export.Listing.Utility.ClsExportStart(_Setting)

		obj.ExportCSVFromFARListing(Me.txt_SQLQuery.Text)
	...
	Sub StartPrinting()
		...
		Dim _Setting As New ClsLLFARSearchPrintSetting With {.DbConnString2Open = m_InitializationData.MDData.MDDbConn,
			...
			.frmhwnd = GetHwnd,
			.FORResor = Chk_Resor.Checked,
			.ShowAsDesign = Me.bPrintAsDesign,
			.ListFilterBez = New List(Of String)(New String() {String.Format("{0}", strFilter)}
			)
																													}
		Dim obj As New FARSearchListing.ClsPrintFARSearchList(_Setting)
		obj.PrintFARSearchList()
	...
	Private Sub Cbo_FARListeBeruf_QueryPopUp(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles Cbo_FARListeBeruf.QueryPopUp
		ListFARListeGAVBeruf(Me.Cbo_FARListeBeruf, Val(Cbo_JahrVon.EditValue), Val(Cbo_VMonth_1.EditValue), Val(Cbo_BMonth_1.EditValue), Chk_Resor.Checked)
	End Sub
