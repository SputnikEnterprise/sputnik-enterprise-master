...
#Region "Fileexlorer..."
	...
	Sub ShowFolderBrowser()
		...
		If dialog.ShowDialog() = DialogResult.OK Then
			Dim a As Action = Function() InlineAssignHelper(Me.txtFilename.Text, String.Format("{0}{1}Aba_Export.CSB", dialog.SelectedPath, _
...
#End Region
	...
	Public Sub New(ByVal _Setting As ClsCSVSettings, ByVal _init As SP.Infrastructure.Initialization.InitializeClass)
		...
		If _ExportSetting.ModulName.ToLower = "abalo" Then Me.txtFilename.Text = If(String.IsNullOrWhiteSpace(_Setting.ExportFileName), My.Settings.Filename4AbaLO, _Setting.ExportFileName)
		If _ExportSetting.ModulName.ToLower = "abaop" Then Me.txtFilename.Text = If(String.IsNullOrWhiteSpace(_Setting.ExportFileName), My.Settings.Filename4AbaOP, _Setting.ExportFileName)
		If _ExportSetting.ModulName.ToLower = "abaze" Then Me.txtFilename.Text = If(String.IsNullOrWhiteSpace(_Setting.ExportFileName), My.Settings.Filename4AbaZE, _Setting.ExportFileName)

		Try
			Dim xDoc As XDocument = XDocument.Load(_ClsProgSetting.GetMDData_XMLFile)
			Dim abaConfigQuery = (From exportSetting In xDoc.Root.Elements("Interfaces")
																				Select New With {
	...
	Private Sub frmAbacus_Disposed(sender As Object, e As System.EventArgs) Handles Me.Disposed

		If Not Me.WindowState = FormWindowState.Minimized Then
			My.Settings.frmAbaLocation = String.Format("{0};{1}", Me.Location.X, Me.Location.Y)
			My.Settings.iAbaWidth = Me.Width
			My.Settings.iAbaHeight = Me.Height
			My.Settings.TrennzeichenAba = Me.cbo_Trennzeichen.Text

			If _ExportSetting.ModulName.ToLower = "abalo" Then My.Settings.Filename4AbaLO = Me.txtFilename.Text
			If _ExportSetting.ModulName.ToLower = "abaop" Then My.Settings.Filename4AbaOP = Me.txtFilename.Text
			If _ExportSetting.ModulName.ToLower = "abaze" Then My.Settings.Filename4AbaZE = Me.txtFilename.Text
	...
	Private Sub frmAbacus_Load(sender As Object, e As System.EventArgs) Handles Me.Load
		...
		Try
			...
			Try
				Me.Width = Math.Max(My.Settings.iAbaWidth, Me.Width)
				Me.Height = Math.Max(My.Settings.iAbaHeight, Me.Height)

				If My.Settings.frmAbaLocation <> String.Empty Then
					Dim aLoc As String() = My.Settings.frmAbaLocation.Split(CChar(";"))
	...
	Private Sub cmdOK_Click(sender As System.Object, e As System.EventArgs) Handles cmdOK.Click
		...
		If String.IsNullOrWhiteSpace(Me.txtFilename.Text) Then Me.txtFilename.Text = String.Format("{0}Aba_Export.CSB", _ClsProgSetting.GetSpSTempPath)
		...
		If Me._ExportSetting.ModulName.ToLower = "abalo" Then
			Dim obj As New ExportAba.ClsExportLOInAba(_ExportSetting, m_InitializationData)
			strResult = obj.GetLOValue4Abacus()

		ElseIf Me._ExportSetting.ModulName.ToLower = "abaop" Then
			Dim obj As New ExportAba.sClsExportOPInAba(_ExportSetting)
			strResult = obj.GetOPValue4Abacus()

		ElseIf Me._ExportSetting.ModulName.ToLower = "abaze" Then
			Dim obj As New ExportAba.sClsExportZEInAba(_ExportSetting, m_InitializationData)
			strResult = obj.GetZEValue4Abacus()
	...
	Sub SaveDataIntoXMLFile()
		...
		If xNode IsNot Nothing Then
			...
			End If
			Dim strNode As String = String.Format("aba{0}fieldtrennzeichen", Me._ExportSetting.ModulName.Substring(3, 2))
			...
			InsertTextNode(xDoc, xElmntFamily, strNode, Me.cbo_Trennzeichen.Text)

			strNode = String.Format("aba{0}darstellungszeichen", Me._ExportSetting.ModulName.Substring(3, 2))
			If xElmntFamily.SelectSingleNode(strNode) IsNot Nothing Then _
						xElmntFamily.RemoveChild(xElmntFamily.SelectSingleNode(strNode))
			InsertTextNode(xDoc, xElmntFamily, strNode, Me.cbo_Darstellungszeichen.Text)

			strNode = String.Format("aba{0}refnr", Me._ExportSetting.ModulName.Substring(3, 2))
			If xElmntFamily.SelectSingleNode(strNode) IsNot Nothing Then _
						xElmntFamily.RemoveChild(xElmntFamily.SelectSingleNode(strNode))
			InsertTextNode(xDoc, xElmntFamily, strNode, Me.chkRef.Checked)

			strNode = String.Format("aba{0}gegenkonto", Me._ExportSetting.ModulName.Substring(3, 2))
			If xElmntFamily.SelectSingleNode(strNode) IsNot Nothing Then _
						xElmntFamily.RemoveChild(xElmntFamily.SelectSingleNode(strNode))
			InsertTextNode(xDoc, xElmntFamily, strNode, Me.chkGegenkostenart.Checked)

			strNode = String.Format("aba{0}mwstcode", Me._ExportSetting.ModulName.Substring(3, 2))
			If xElmntFamily.SelectSingleNode(strNode) IsNot Nothing Then _
						xElmntFamily.RemoveChild(xElmntFamily.SelectSingleNode(strNode))
			InsertTextNode(xDoc, xElmntFamily, strNode, Me.txtMwStCode.Text)
