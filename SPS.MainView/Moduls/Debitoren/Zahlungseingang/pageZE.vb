...
#Region "private consts"
	...
	Private Const MODUL_NAME = "Zahlungseingang"
	Private Const ProgHeaderName As String = "Sputnik Enterprise Suite - [{0}]"
	Private Const UsercontrolCaption As String = "Zahlungseingänge"

	Private Const MODUL_NAME_SETTING = "ZE"
...
#Region "private fields"
	...
	Private m_Setting As ClsZESetting
...
#Region "Private property"
	...
	Private ReadOnly Property SelectedRowViewData As FoundedCustomerROPData
		Get
			...
			If Not (grdView Is Nothing) Then
				...
				If (selectedRows.Count > 0) Then
					Dim contact = CType(grdView.GetRow(selectedRows(0)), FoundedCustomerROPData)
					Return contact
				End If
...
#Region "Private methods"
	...
	Private Sub SetFormLayout()
		...
		End Try
		Dim setting_splitterpos = m_SettingsManager.ReadInteger(SettingZEKeys.SETTING_ZE_SCC_HEADERINFO_SPLITTERPOSION)
		...
		setting_splitterpos = m_SettingsManager.ReadInteger(SettingZEKeys.SETTING_ZE_SCC_MAIN_SPLITTERPOSION)
	...
	Sub SaveFormProperties()
		...
		m_SettingsManager.WriteInteger(SettingZEKeys.SETTING_ZE_SCC_MAIN_SPLITTERPOSION, Me.sccMain.SplitterPosition)
		m_SettingsManager.WriteInteger(SettingZEKeys.SETTING_ZE_SCC_HEADERINFO_SPLITTERPOSION, Me.sccHeaderInfo.SplitterPosition)
...
#End Region
	...
	Private Sub ResetPropertyGrid()
		...
		'Try
		'	For Each itm In m_griddata.TopPropertyColumnData
	...
	Private Function LoadFoundedCustomerList() As Boolean
		...
		Dim listOfCustomerBill = m_DataAccess.GetDbCustomerRecipientOfPaymentsData4Show(m_griddata.SQLQuery)
		If listOfCustomerBill Is Nothing Then Return False

		Dim responsiblePersonsGridData = (From person In listOfCustomerBill
										  Select New FoundedCustomerROPData With
					 {.mdnr = person.mdnr, .customermdnr = person.customermdnr,
						.zenr = person.zenr,
						.renr = person.renr,
						.kdnr = person.kdnr,
						._res = person._res,
		...
		Dim listDataSource As BindingList(Of FoundedCustomerROPData) = New BindingList(Of FoundedCustomerROPData)
	...
	Private Sub FillPopupFields(ByVal selectedrow As FoundedCustomerROPData)
		...
		Try
			...
			Else
				Dim listOfData As New List(Of FoundedCustomerROPData)
	...
	Sub OngvMain_RowCellClick(sender As System.Object, e As DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs)
		...
		If (e.Clicks = 2) Then
			...
			If Not dataRow Is Nothing Then
				Dim viewData = CType(dataRow, FoundedCustomerROPData)

				Select Case column.Name.ToLower
					Case "kdnr", "firma1", "firma2"
						If viewData.kdnr > 0 Then
							Dim _ClsKD As New ClsOpenModul(New ClsSetting With {.SelectedMDNr = viewData.mdnr, .SelectedKDNr = Me.m_Setting.SelectedKDNr})
					...
					Case Else
						If viewData.zenr > 0 Then
							Dim _Clsre As New ClsOpenModul(New ClsSetting With {.SelectedMDNr = viewData.mdnr, .SelectedZENr = viewData.zenr})
							_Clsre.OpenSelectedPayment()
						End If
...
#Region "DropDown Button für New und Print..."
	...
	' ''' <param name="e"></param>
	' ''' <remarks></remarks>
	'Private Sub cmdPrint_Click(sender As System.Object, e As System.EventArgs)
...
#Region "Helpers"
	...
	Private Sub BuildNewPrintContextMenu()
		...
		Try
			' build contextmenu
			Dim _ClsNewPrint As New ClsZEModuls(Me.m_Setting)
