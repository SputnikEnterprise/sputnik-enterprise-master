...
Public Class pageMahnung


#Region "private consts"

	Private Const MODUL_NAME = "Mahnungen"
	Private Const ProgHeaderName As String = "Sputnik Enterprise Suite - [{0}]"
	Private Const UsercontrolCaption As String = "Mahnungen"

	Private Const MODUL_NAME_SETTING = "Mahnung"
...
#Region "private fields"
	...
	Private m_Setting As ClsMahnSetting
...
#Region "Private property"
	...
	Private ReadOnly Property SelectedRowViewData As FoundedCustomerreminderData
		Get
			...
			If Not (grdView Is Nothing) Then
				...
				If (selectedRows.Count > 0) Then
					Dim contact = CType(grdView.GetRow(selectedRows(0)), FoundedCustomerreminderData)
					Return contact
				End If
...
#Region "Contructor"
	...
	Public Sub New()
		...
		Try
			...
			m_SettingsManager = New SettingsMahnManager
			Me.m_Setting = New ClsMahnSetting
...
#Region "Private methods"
	...
	Private Sub SetFormLayout()
		...
		End Try
		Dim setting_splitterpos = m_SettingsManager.ReadInteger(SettingMahnKeys.SETTING_MAHN_SCC_HEADERINFO_SPLITTERPOSION)
		...
		setting_splitterpos = m_SettingsManager.ReadInteger(SettingMahnKeys.SETTING_MAHN_SCC_MAIN_SPLITTERPOSION)
	...
	Sub SaveFormProperties()
		...
		sccHeaderInfo.SplitterPosition = sccHeaderInfo.Width - 250
		m_SettingsManager.WriteInteger(SettingMahnKeys.SETTING_MAHN_SCC_MAIN_SPLITTERPOSION, Me.sccMain.SplitterPosition)
		m_SettingsManager.WriteInteger(SettingMahnKeys.SETTING_MAHN_SCC_HEADERINFO_SPLITTERPOSION, Me.sccHeaderInfo.SplitterPosition)
...
#End Region
	...
	Private Sub ResetMainGrid()
		...
		Try
			...
			'ElseIf column.FieldName.Contains("renr") Or column.FieldName.Contains("esnr") Or column.FieldName.Contains("kdnr") Or column.FieldName.Contains("manr") Then
			'column.OptionsFilter.AutoFilterCondition = DevExpress.XtraGrid.Columns.AutoFilterCondition.Default
			'		column.AppearanceHeader.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Far
			'		column.AppearanceHeader.Options.UseTextOptions = True
	...
	Private Function LoadFoundedCustomerreminderList() As Boolean
		Dim m_DataAccess As New MainGrid

		Dim listOfCustomerBill = m_DataAccess.GetDbCustomerreminderData4Show(m_griddata.SQLQuery)
		If listOfCustomerBill Is Nothing Then Return False

		Dim responsiblePersonsGridData = (From person In listOfCustomerBill
										  Select New FoundedCustomerreminderData With
					 {.renr = person.renr,
						.kdnr = person.kdnr,
						._res = person._res,
						...
						.reminder_0date = person.reminder_0date,
						.reminder_1date = person.reminder_1date,
						.reminder_2date = person.reminder_2date,
						.reminder_3date = person.reminder_3date,
		...
		Dim listDataSource As BindingList(Of FoundedCustomerreminderData) = New BindingList(Of FoundedCustomerreminderData)
	...
	Private Sub frm_Load(sender As Object, e As System.EventArgs) Handles Me.Load
		...
		LoadFoundedCustomerreminderList()
	...
	'			Catch ex As Exception
	'				m_Logger.LogError(String.Format("{0}.Spaltenwert ermitteln: {1}", strPFieldName.Split(CChar(";"))(i).ToLower, ex.Message))
	...
	'			itm.Caption = String.Format("{0}{1}{2}", _
	'																	m_translate.GetSafeTranslationValue(Str.Replace("__", "")) & _
	...
	Private Function GetCellValue(ByVal strCellName As String, ByVal selectedrow As FoundedCustomerreminderData) As MenuData
	...
	Sub RunExtraFunctions4Listing(ByVal strFuncname As String)
		...
		Select Case strFuncname.ToUpper
			Case "KDName".ToUpper
				Dim _ClsPropose As New ClsOpenModul(New ClsSetting With {.SelectedKDNr = Me.m_Setting.SelectedKDNr})
	...
	Private Sub FillPopupFields(ByVal selectedrow As FoundedCustomerreminderData)
		...
		Try
			...
			Else
				Dim listOfData As New List(Of FoundedCustomerreminderData)
	...
	Sub OngvMain_RowCellClick(sender As System.Object, e As DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs)
		...
		If (e.Clicks = 2) Then
			...
			If Not dataRow Is Nothing Then
				Dim viewData = CType(dataRow, FoundedCustomerreminderData)
				...
				Select Case column.Name.ToLower
					...
					Case Else
						If viewData.renr > 0 Then
							Dim _Clsre As New ClsOpenModul(New ClsSetting With {.SelectedRENr = viewData.renr})
							_Clsre.OpenSelectedInvoice(ModulConstants.MDData.MDNr, ModulConstants.UserData.UserNr)
						End If
...
#Region "Helpers"
	...
	Private Sub BuildNewPrintContextMenu()
		...
		Try
			' build contextmenu
			Dim _ClsNewPrint As New ClsMahnModuls(Me.m_Setting)
