...
#Region "Public Methods"
		...
		Public Function LoadInvoiceData(ByVal invoiceNumber As Integer) As Boolean
			...
			success = success AndAlso LoadOutgoingEMailsData(m_invoiceData.ReNr)
		...
		Public Sub SaveInvoiceData()
			...
			If (ValidateData()) Then
				...
				If IsInvoiceDataLoaded Then
					...
					Dim customerNoticeData = m_CustomerDatabaseAccess.LoadCustomerNoticesData(m_invoiceData.KdNr)
					If Not customerNoticeData Is Nothing Then
						customerNoticeData.Notice_Invoice = txtInvoiceComment.EditValue
						customerNoticeData.ChangedFrom = m_InitializationData.UserData.UserFullName

						success = success AndAlso m_CustomerDatabaseAccess.UpdateCustomerNoticesData(customerNoticeData)
					End If
				...
				If (success) Then
					...
					DevExpress.XtraEditors.XtraMessageBox.Show((m_Translate.GetSafeTranslationValue("Die Daten wurden gespeichert.")), m_Translate.GetSafeTranslationValue("Daten speichern"), MessageBoxButtons.OK, MessageBoxIcon.Information)
				Else
		...
		Public Sub DeleteInvoice()
			...
			Else
				strFileName = Path.Combine(m_InitializationData.UserData.spTempInvoicePath, String.Format("Rechnungen {0}.ex", m_InitializationData.MDData.MDName, m_InitializationData.MDData.MDCity)) ' result.JobResultInvoiceData(0).CustomerNumber))
				strFileName = Path.ChangeExtension(strFileName, ".pdf")

				If File.Exists(strFileName) Then
					Try
						File.Delete(strFileName)
					Catch ex As Exception
						strFileName = Path.Combine(m_InitializationData.UserData.spTempInvoicePath, String.Format("{0} - {1}.ex", Path.GetRandomFileName, exportResult.JobResultInvoiceData(0).CustomerNumber))
						strFileName = Path.ChangeExtension(strFileName, ".pdf")
				...
				For Each itm In exportResult.JobResultInvoiceData
				...
				If exportResult.JobResultInvoiceData.Count > 1 Then
					...
					Dim success = pdfUtility.MergePdfFiles(fileList.ToArray, strFileName)
				Else
					strFileName = exportResult.JobResultInvoiceData(0).ExportedFileName
				End If
			...
			If Not String.IsNullOrWhiteSpace(strFileName) Then
				...
				result = m_InvoiceDatabaseAccess.DeleteInvoiceAndInsertInvoiceDocumentIntoDeleteDb(m_invoiceData.Id, ConstantValues.ModulName, m_InitializationData.UserData.UserFullNameWithComma, m_InitializationData.UserData.UserNr, If(fileByte.Length = 0, Nothing, fileByte))
			Else
				result = m_InvoiceDatabaseAccess.DeleteInvoice(m_invoiceData.Id, ConstantValues.ModulName, m_InitializationData.UserData.UserFullNameWithComma, m_InitializationData.UserData.UserNr)
...
#Region "Reset Form"
		...
		Private Sub Reset()
			...
			txtBetragInk.Text = String.Empty
			txtBetragOffen.Text = String.Empty
			txtExklusiv.Text = String.Empty
			...
			txtMwSt.Text = String.Empty
			txtMwStFrei.Text = String.Empty
			...
			txtFibuHaben0.Text = String.Empty
			txtFibuHaben1.Text = String.Empty
			txtFibuSoll.Text = String.Empty
			...
			tpAddress.SelectedPage = tnpEMailData
		...
		Private Sub ResetOutgoingEMailsGrid()

			gvOutgoingEMails.BorderStyle = BorderStyles.NoBorder
			gvOutgoingEMails.OptionsView.ShowIndicator = False

			gvOutgoingEMails.OptionsView.ShowColumnHeaders = True ' False
			'gvOutgoingEMails.OptionsView.ShowVerticalLines = DevExpress.Utils.DefaultBoolean.False
			'gvOutgoingEMails.OptionsView.ShowHorizontalLines = DevExpress.Utils.DefaultBoolean.False
			gvOutgoingEMails.FocusRectStyle = DevExpress.XtraGrid.Views.Grid.DrawFocusRectStyle.RowFullFocus '  DevExpress.XtraGrid.Views.Grid.DrawFocusRectStyle.None
			'gvOutgoingEMails.OptionsSelection.EnableAppearanceFocusedRow = False
			'gvOutgoingEMails.OptionsSelection.EnableAppearanceHideSelection = False


			' Reset the grid
			gvOutgoingEMails.Columns.Clear()

			Dim columnCreatedOn As New DevExpress.XtraGrid.Columns.GridColumn()
			columnCreatedOn.Caption = "Datum"
			...
			'columnCreatedOn.AppearanceHeader.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Far
			'columnCreatedOn.AppearanceHeader.Options.UseTextOptions = True
			'columnCreatedOn.AppearanceCell.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Far
			'columnCreatedOn.AppearanceCell.Options.UseTextOptions = True
			columnCreatedOn.Width = 40
			gvOutgoingEMails.Columns.Add(columnCreatedOn)

			Dim columnSender As New DevExpress.XtraGrid.Columns.GridColumn()
			columnSender.Caption = "Absender"
			columnSender.Name = "Sender"
			columnSender.FieldName = "Sender"
			columnSender.Visible = False
			gvOutgoingEMails.Columns.Add(columnSender)

			Dim columnReceiver As New DevExpress.XtraGrid.Columns.GridColumn()
			columnReceiver.Caption = "Empfänger"
			columnReceiver.Name = "Receiver"
			columnReceiver.FieldName = "Receiver"
			columnReceiver.Visible = True
			columnReceiver.Width = 40
			gvOutgoingEMails.Columns.Add(columnReceiver)

			grdOutgoingEMails.DataSource = Nothing
...
#Region "Load master Data"
		...
		Private Sub LoadDebitorenartDropDown()
			...
			lueDebitorenart.Properties.DataSource = debitorenart
			...
			Try
				...
				Else
					...
					For Each customerAssignedBranch In customerAssignedBranchData
						...
						If customerBranchCode.HasValue AndAlso customerBranchCode <> 0 Then
							...
							If Not extistingBranch Is Nothing Then
								...
								mergedBranchData.Add(New BranchViewData With {.Branche = extistingBranch.Branche, .TranslatedBrancheText = extistingBranch.TranslatedBrancheText, .IsAssignedToCustomer = True})
							...
							Else
								...
								mergedBranchData.Add(New BranchViewData With {.Branche = customerAssignedBranch.Description, .TranslatedBrancheText = customerAssignedBranch.Description, .IsAssignedToCustomer = True})
					...
					For Each branch In allBranchData
						mergedBranchData.Add(New BranchViewData With {.Branche = branch.Branche, .TranslatedBrancheText = branch.TranslatedBrancheText, .IsAssignedToCustomer = False})
					Next
				End If
...
#Region "Load, save Invoice Data"
		...
		Private Function ValidateData() As Boolean
			...
			Try
				...
				valid = valid And SetErrorIfInvalid(txtEMail, errorProvider, Not String.IsNullOrWhiteSpace(txtEMail.EditValue) AndAlso Not eMailPattern.IsMatch(txtEMail.EditValue), errorEMailAddress)
		...
		Private Sub MapInvoiceDataToUi()
			...
			m_selectedAddress = New DataObjects.CustomerReAddress With {
									...
									.REeMail = m_invoiceData.ReMail,
									.SendAsZip = m_invoiceData.SendAsZip,
			...
			ElseIf m_invoiceData.Art.ToUpper = "A" Then
				CType(lueDebitorenart.Properties.DataSource, List(Of Debitorenart)).Add(
			...
			lueDebitorenart.EditValue = debitorenart
			lueBankdaten.EditValue = m_invoiceData.ESRBankName
			daeDatum.EditValue = m_invoiceData.FakDat
			...
			' Beträge
			txtMwStFrei.Text = m_invoiceData.BetragOhne
			txtExklusiv.Text = m_invoiceData.BetragEx
			txtMwSt.Text = m_invoiceData.MWST1
			txtBetragInk.Text = m_invoiceData.BetragInk
			txtBetragBezahlt.Text = m_invoiceData.Bezahlt
			txtBetragOffen.Text = m_invoiceData.BetragInk - m_invoiceData.Bezahlt

			txtFibuSoll.EditValue = m_invoiceData.FKSoll
			txtFibuHaben0.EditValue = m_invoiceData.FKHaben0
		...
		Private Sub MapUiToInvoiceData()
			...
			With m_invoiceData
				...
				.Lp = CType(daeDatum.EditValue, Date).Date.Month
				.FakDat = CType(daeDatum.EditValue, Date).Date
				...
				.ReMail = txtEMail.Text
				.SendAsZip = tgsSendAsZIP.EditValue
		...
		Private Function LoadOutgoingEMailsData(ByVal invoiceNumber As Integer) As Boolean
			...
			mailData.Number = invoiceNumber
			Dim data = m_ListingDatabaseAccess.LoadAssignedModulOutgoingEMailData(m_InitializationData.MDData.MDGuid, ModulNumberEnum.INVOICE, invoiceNumber)
			If data Is Nothing Then Return True

			Dim listDataSource As BindingList(Of OutgoingEMailData) = New BindingList(Of OutgoingEMailData)

			' Convert the data to view data.
			For Each p In data

				Dim cViewData = New OutgoingEMailData() With {.Customer_ID = p.Customer_ID,
						.CreatedFrom = p.CreatedFrom,
						.CreatedOn = p.CreatedOn,
						.CreatedUserNumber = p.CreatedUserNumber,
						.ID = p.ID,
						.ModulNumber = p.ModulNumber,
						.Number = p.Number,
						.Receiver = p.Receiver,
						.Sender = p.Sender}
			...
			Dim wos = New SP.Internal.Automations.WOSUtility.CustomerExport(m_InitializationData)
			Dim wosNotificationdata = wos.PerformWOSModulNotificationlistWebservice(ModulNumberEnum.INVOICE, invoiceNumber)
			If wosNotificationdata Is Nothing Then Return True

			For Each itm In wosNotificationdata
				Dim cViewData = New OutgoingEMailData() With {.Customer_ID = itm.Customer_ID,
						.CreatedFrom = Nothing,
						.CreatedOn = itm.CreatedOn,
						.CreatedUserNumber = Nothing,
						.ID = itm.ID,
						.ModulNumber = ModulNumberEnum.INVOICE,
			...
			grdOutgoingEMails.DataSource = listDataSource
			grdOutgoingEMails.ForceInitialize()

			Return result
		End Function

		Private Sub OngvOutgoingEMails_RowStyle(sender As System.Object, e As DevExpress.XtraGrid.Views.Grid.RowStyleEventArgs) Handles gvOutgoingEMails.RowStyle

			If e.RowHandle >= 0 Then

				Dim rowData = CType(gvOutgoingEMails.GetRow(e.RowHandle), OutgoingEMailData)
				If rowData Is Nothing Then Return

				If rowData.CreatedFrom Is Nothing OrElse String.IsNullOrWhiteSpace(rowData.CreatedFrom) Then
		...
		''' </summary>
		Private Sub OngvInvoiceRows_DoubleClick(sender As System.Object, e As System.EventArgs) Handles gvInvoiceRows.DoubleClick
		...
		''' <param name="e"></param>
		''' <remarks></remarks>
		Private Sub OnbtnShow_Payments(sender As Object, e As EventArgs) Handles btnShowPayments.Click
...
#Region "Event Handles"
		...
		Private Sub OnnbMain_LinkClicked(ByVal sender As Object,
			...
			Try
				...
				Select Case strLinkName.ToLower
					...
					Case "email_Invoice_Data".ToLower
		...
		''' </summary>
		Private Sub OndaeDatumEditValueChanged(sender As System.Object, e As System.EventArgs) Handles daeDatum.EditValueChanged
			CalculateDueDate()
		End Sub
		...
		''' </summary>
		Private Sub OnluePostcodeProcessNewValue(sender As System.Object, e As DevExpress.XtraEditors.Controls.ProcessNewValueEventArgs) Handles luePostcode.ProcessNewValue
		...
		''' </summary>
		Private Sub OnluePostcodeEditValueChanged(sender As System.Object, e As System.EventArgs) Handles luePostcode.EditValueChanged
		...
		''' </summary>
		Private Sub OnlueDebitorenartEditValueChanged(sender As System.Object, e As System.EventArgs) Handles lueDebitorenart.EditValueChanged
			If lueDebitorenart.Properties.ReadOnly Then
				Return
			End If
			Select Case lueDebitorenart.EditValue
				Case "A"
					txtFibuHaben1.Text = m_postingAccounts(2)
					txtFibuHaben0.Text = m_postingAccounts(4)
				Case "I"
					txtFibuHaben1.Text = m_postingAccounts(3)
					txtFibuHaben0.Text = m_postingAccounts(5)
				Case "F"
					txtFibuHaben1.Text = m_postingAccounts(17)
					txtFibuHaben0.Text = m_postingAccounts(18)
				Case "GA"
					txtFibuHaben1.Text = m_postingAccounts(33)
					txtFibuHaben0.Text = m_postingAccounts(34)
				Case "GI"
					txtFibuHaben1.Text = m_postingAccounts(35)
					txtFibuHaben0.Text = m_postingAccounts(36)
				Case "GF"
					txtFibuHaben1.Text = m_postingAccounts(37)
					txtFibuHaben0.Text = m_postingAccounts(38)
				Case "RA"
					txtFibuHaben1.Text = m_postingAccounts(23)
					txtFibuHaben0.Text = m_postingAccounts(24)
				Case "RI"
					txtFibuHaben1.Text = m_postingAccounts(27)
					txtFibuHaben0.Text = m_postingAccounts(28)
				Case "RF"
					txtFibuHaben1.Text = m_postingAccounts(31)
					txtFibuHaben0.Text = m_postingAccounts(32)
			End Select

			If String.IsNullOrWhiteSpace(txtFibuHaben1.Text) OrElse String.IsNullOrWhiteSpace(txtFibuHaben0.Text) Then
				m_UtilityUI.ShowErrorDialog("Achtung: Sie haben für Debitoren '" + lueDebitorenart.EditValue + "' keine Hauptkonto definiert. " +
		...
		''' </summary>
		Private Sub chkAllowedTab_EditValueChanged(sender As Object, e As EventArgs) Handles chkAllowedTab.EditValueChanged
		...
		''' </summary>
		Private Sub OngvInvoiceRowsFocusedRowChanged(sender As System.Object, e As DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs) Handles gvInvoiceRows.FocusedRowChanged
			m_newInvoiceRow = Nothing
			SetInvoiceRowUi()
		End Sub
		...
		''' </summary>
		Private Sub OngridInvoiceRows_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles gridInvoiceRows.KeyDown
		...
		''' </summary>
		Private Sub OnTxtRowValueExclEditValueChanged(sender As System.Object, e As System.EventArgs) Handles txtRowValueExcl.Leave
		...
		Private Sub OntxtRowValueExclKeyUp(sender As Object, e As KeyEventArgs) Handles txtRowValueExcl.KeyUp
...
#Region "Helper Methods"
		...
		Private Sub TranslateControls()
			...
			tnpEMailData.Caption = m_Translate.GetSafeTranslationValue(tnpEMailData.Caption)
			tnpSonstiges.Caption = m_Translate.GetSafeTranslationValue(tnpSonstiges.Caption)
			...
			lblzahlungskondition.Text = m_Translate.GetSafeTranslationValue(lblzahlungskondition.Text)
			...
			lblExklusiv.Text = m_Translate.GetSafeTranslationValue(lblExklusiv.Text)
			...
			lblFibuSoll.Text = m_Translate.GetSafeTranslationValue(lblFibuSoll.Text)
			...
			lblKontoauszug.Text = m_Translate.GetSafeTranslationValue(lblKontoauszug.Text)
			...
			lblMwSt.Text = m_Translate.GetSafeTranslationValue(lblMwSt.Text)
			lblMwStFrei.Text = m_Translate.GetSafeTranslationValue(lblMwStFrei.Text)
			...
			lblEMailAdresse.Text = m_Translate.GetSafeTranslationValue(lblEMailAdresse.Text)
			lblIndividualFiles.Text = m_Translate.GetSafeTranslationValue(lblIndividualFiles.Text)
			tgsSendAsZIP.Properties.OnText = m_Translate.GetSafeTranslationValue(tgsSendAsZIP.Properties.OnText)
			tgsSendAsZIP.Properties.OffText = m_Translate.GetSafeTranslationValue(tgsSendAsZIP.Properties.OffText)
			...
			groupInvoiceRows.Text = m_Translate.GetSafeTranslationValue(groupInvoiceRows.Text)
		...
		Private Sub CreateMyNavBar()
			...
			Try
				...
				Dim send_eMail_P_Data = New NavBarItem(m_Translate.GetSafeTranslationValue("Via E-Mail senden"))
				send_eMail_P_Data.Name = "email_Invoice_Data"
				send_eMail_P_Data.SmallImage = Me.ImageCollection1.Images(11)
				...
				Try
					...
					groupExtra.ItemLinks.Add(send_eMail_P_Data)
					send_eMail_P_Data.Enabled = m_md.AllowedExportEmployee2WOS(m_InitializationData.MDData.MDNr, Now.Year)
		...
		Private Sub SwitchReadOnlyFields(ByVal isReadOnly As Boolean)
			...
			lueDebitorenart.Properties.ReadOnly = isReadOnly
		...
		Private Sub DisplayAdress()
			...
			Else
				...
				txtEMail.Text = m_selectedAddress.REeMail
				tgsSendAsZIP.EditValue = m_selectedAddress.SendAsZip
		...
		Private Sub SendInvoiceWithEMail()
			If String.IsNullOrWhiteSpace(txtEMail.EditValue) Then
				m_UtilityUI.ShowOKDialog(m_Translate.GetSafeTranslationValue("Sie haben keine E-Mail-Adresse definiert."), "E-Mail", MessageBoxIcon.Exclamation)

				Return
			End If

			If Not m_invoiceNumber.HasValue Then Return
			If Not IsUserActionAllowed(m_InitializationData.UserData.UserNr, 406, m_InitializationData.MDData.MDNr) Then Return
			...
			Try
				...
				Dim invoiceNumbers As New List(Of Integer) From {m_invoiceNumber}

				Dim _setting As New SPS.Listing.Print.Utility.InvoicePrint.InvoicePrintData With {.frmhwnd = Me.Handle, .InvoiceNumbers = invoiceNumbers, .PrintInvoiceAsCopy = False, .ShowAsDesign = False, .ExportPrintInFiles = True}
				Dim printUtil = New ClsPrintInvoice(m_InitializationData)
				printUtil.PrintData = _setting
				Dim result = printUtil.PrintInvoice()
				...
				Dim preselectionSetting As New PreselectionMailData With {.MailType = MailTypeEnum.INVOICE, .InvoiceNumber = m_invoiceNumber, .CustomerNumber = m_selectedCustomer.KDNr, .PDFFilesToSend = New List(Of String) From {filename}}
		...
		Private Sub SendSelectedInvoiceToWOS()
			...
			Try
				...
				Dim _setting As New SPS.Listing.Print.Utility.InvoicePrint.InvoicePrintData With {.frmhwnd = Me.Handle,
																									 ...
																									 .PrintInvoiceAsCopy = False,
																									 .ShowAsDesign = False,
																									 .WOSSendValueEnum = SPS.Listing.Print.Utility.InvoicePrint.InvoicePrintData.WOSValue.PrintOtherSendWOS}
				Dim printUtil = New ClsPrintInvoice(m_InitializationData)
				printUtil.PrintData = _setting
				Dim result = printUtil.PrintInvoice()
		...
		Private Sub BookoutBonus()
			...
			If (m_invoiceData.Bezahlt = m_invoiceData.BetragInk) Then
				...
				lblAusgleichon.Text = String.Empty
			...
			Else
				...
				lblAusgleichon.Text = String.Format(m_Translate.GetSafeTranslationValue("Ausgeglichen am: {0}"), m_invoiceData.GebuchtAm)
		...
		Private Sub PrepareStatusAndNavigationBar(ByVal invoice As DataObjects.Invoice)
			...
			If Not customerMasterData Is Nothing Then
				m_Wos_P_Data.Enabled = customerMasterData.sendToWOS.GetValueOrDefault(False)
			Else
				m_Wos_P_Data.Enabled = False
			End If
			...
			If m_Guthaben_NavbarItem.Visible AndAlso m_Guthaben_NavbarItem.Enabled Then
				If m_invoiceData.Bezahlt = m_invoiceData.BetragInk Then
					m_Guthaben_NavbarItem.Caption = m_Translate.GetSafeTranslationValue("Ausgleich löschen")
					lblAusgleichon.Text = String.Format(m_Translate.GetSafeTranslationValue("Ausgeglichen am: {0}"), m_invoiceData.GebuchtAm)
				Else
			...
			tnpNotice_Invoice.PageVisible = Not String.IsNullOrWhiteSpace(customerMasterData.Notice_Invoice)
			txtInvoiceComment.EditValue = customerMasterData.Notice_Invoice

			CType(tpAddress, INavigationPane).ButtonsPanel.Buttons(4).Properties.Appearance.ForeColor = If(String.IsNullOrWhiteSpace(txtInvoiceComment.EditValue), Color.Black, Color.Orange)
			CType(tpAddress, INavigationPane).ButtonsPanel.Buttons(4).Properties.Appearance.Font = New Font(tpAddress.AppearanceButton.Normal.Font, If(String.IsNullOrWhiteSpace(txtInvoiceComment.EditValue), FontStyle.Regular, FontStyle.Bold))
