...
Namespace RichEditSendMail
	...
	Public Class frmPayrollEMailMerge
...
#Region "private fields"
		...
		Private m_CurrentPayrollMailMergeData As New PayrollEMailMergeViewData
...
#End Region
		...
		Private ReadOnly Property SelectedMergeRecord As PayrollEMailMergeViewData
			Get
				...
				If Not (gvRP Is Nothing) Then
					...
					If (selectedRows.Count > 0) Then
						Dim doc = CType(gvRP.GetRow(selectedRows(0)), PayrollEMailMergeViewData)
...
#Region "public property"
		...
		Public Property PayrollEMailMergeData As List(Of PayrollEMailMergeData)
...
#Region "public methodes"
		...
		Public Function LoadData() As Boolean
			...
			If m_PayrollNumber.GetValueOrDefault(0) > 0 Then success = success AndAlso LoadPayrollData()

			LoadModulSettingData()
			If PreselectionData.MailType = MailTypeEnum.MOREPAYROLLS Then
				LoadPayrollMailMergeData()
			End If
...
#End Region
		...
		Private Sub ResetMergeDataGrid()
			...
			gvMergeData.Columns.Add(columnEmployeeNumber)
			...
			columnEmployeefullnameWithComma.OptionsColumn.AllowEdit = False
			...
			gvMergeData.Columns.Add(columnEmployeefullnameWithComma)
			...
			columnEMail.Caption = m_Translate.GetSafeTranslationValue("EMail-Adresse")
			columnEMail.Name = "EMail"
			columnEMail.FieldName = "EMail"
			columnEMail.Visible = True
			gvMergeData.Columns.Add(columnEMail)

			Dim columnNumberOfPayrolls As New DevExpress.XtraGrid.Columns.GridColumn()
			columnNumberOfPayrolls.OptionsFilter.AutoFilterCondition = DevExpress.XtraGrid.Columns.AutoFilterCondition.Contains
			columnNumberOfPayrolls.OptionsColumn.AllowEdit = False
			columnNumberOfPayrolls.Caption = m_Translate.GetSafeTranslationValue("Anzahl Lohnabrechnungen")
			columnNumberOfPayrolls.Name = "NumberOfPayrolls"
			columnNumberOfPayrolls.FieldName = "NumberOfPayrolls"
			columnNumberOfPayrolls.Visible = True
			gvMergeData.Columns.Add(columnNumberOfPayrolls)
		...
		Sub OngvMergeData_RowCellClick(sender As System.Object, e As DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs)
			...
			If (e.Clicks = 2) Then
				...
				Select Case column.Name.ToLower
					...
					Case "EmployeefullnameWithComma".ToLower
						If data.EmployeeNumber.GetValueOrDefault(0) > 0 Then
							Dim hub = MessageService.Instance.Hub
							Dim openMng As New OpenEmployeeMngRequest(Me, m_InitializationData.UserData.UserNr, m_InitializationData.MDData.MDNr, data.EmployeeNumber.GetValueOrDefault(0))
							hub.Publish(openMng)
		...
		Private Sub OngvMergeData_MasterRowGetRelationDisplayCaption(sender As Object, e As MasterRowGetRelationNameEventArgs) Handles gvMergeData.MasterRowGetRelationDisplayCaption
			...
			If e.RelationIndex = 1 Then e.RelationName = "Lohnabrechnungen"
...
#Region "load Database data"
		...
		Private Function LoadCommonEmployeeData() As Boolean
			Dim result As Boolean = True

			If m_EmployeeNumber Is Nothing Then Return False
		...
		Private Function LoadPayrollData() As Boolean

			Try
				m_PayrollData = m_PayrollDatabaseAccess.LoadPayrollMasterData(m_PayrollNumber)
...
#End Region
		...
		Private Function LoadPayrollMailMergeData() As Boolean

			Try
				If PayrollEMailMergeData Is Nothing OrElse PayrollEMailMergeData.Count = 0 Then
					m_UtilityUI.ShowErrorDialog(m_Translate.GetSafeTranslationValue("Lohnabrechnung Daten konnten nicht geladen werden."))

					Return False
				End If
				'aceSendAsIndividualFiles.Visible = Not PayrollEMailMergeData(0).IndividualAttachments Is Nothing

				Dim listDataSource As BindingList(Of PayrollEMailMergeViewData) = New BindingList(Of PayrollEMailMergeViewData)

				' Convert the data to view data.
				For Each p In PayrollEMailMergeData

					Dim cViewData = New PayrollEMailMergeViewData() With {.EmployeeNumber = p.EmployeeNumber,
						.Attachment = p.Attachment,
						.IndividualAttachments = p.IndividualAttachments,
						.PayrollNumbers = p.PayrollNumbers,
						.Lastname = p.Lastname,
						.Firstname = p.Firstname,
						.PayrollDate = p.PayrollDate,
						.PayrollNumber = p.PayrollNumber,
						.NumberOfPayrolls = p.NumberOfPayrolls,
			...
			Return Not PayrollEMailMergeData Is Nothing
		End Function
...
#End If
				...
				m_CurrentPayrollMailMergeData = SelectedMergeRecord
				m_EmployeeNumber = m_CurrentPayrollMailMergeData.EmployeeNumber
				If Not LoadCommonEmployeeData() Then Return

				LoadMailSubjectData()
				LoadmyMailTemplate(Path.Combine(m_TemplatePath, m_AsssignedTemplateFilename))

				If m_CurrentPayrollMailMergeData.IndividualAttachments Is Nothing Then
					attachments = New List(Of String) From {m_CurrentPayrollMailMergeData.Attachment}
				Else
					attachments = New List(Of String)(m_CurrentPayrollMailMergeData.IndividualAttachments)
				End If
		...
		Private Sub OnbtnMergeSend_Click(sender As Object, e As EventArgs) Handles btnMergeSend.Click
			...
			If Not m_UtilityUI.ShowYesNoDialog(String.Format(msg, PayrollEMailMergeData.Count), m_Translate.GetSafeTranslationValue("Serien-Versand")) Then Return
			aceSendResult.Text = String.Empty
		...
		Private Sub FocusData(ByVal reEMail As String)
			...
			Try
				If Not grdMergeData.DataSource Is Nothing Then
					...
					Dim commonViewData = CType(gvMergeData.DataSource, IEnumerable(Of PayrollEMailMergeViewData))

					Dim index = commonViewData.ToList().FindIndex(Function(data) data.EMail = reEMail)
		...
		Private Sub OngvMergeData_CustomUnboundColumnData(sender As System.Object, e As DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs) Handles gvMergeData.CustomUnboundColumnData
			...
			If e.Column.Name = "Processing" Then
				If (e.IsGetData()) Then
					Dim state = CType(e.Row, PayrollEMailMergeViewData).ProcessState
			...
			ElseIf e.Column.Name = "AttachmentAsZip" Then
				If (e.IsGetData()) Then
					Dim state = 0
					Dim data = CType(e.Row, PayrollEMailMergeViewData)
					...
					Select Case state
						...
						Case Else
							e.Value = ImageCollection1.Images(7)
		...
		Private Function BuildAndSendMeragedInvoices() As Boolean
			...
			Try
				Dim dataToSend = CType(gvMergeData.DataSource, IEnumerable(Of PayrollEMailMergeViewData))

				For Each itm In dataToSend
					m_CurrentPayrollMailMergeData = itm
					FocusData(itm.EMail)

					m_EmployeeNumber = m_CurrentPayrollMailMergeData.EmployeeNumber
					If Not LoadCommonEmployeeData() Then Return False
					...
					LoadmyMailTemplate(Path.Combine(m_TemplatePath, m_AsssignedTemplateFilename))
					m_CustomerNumber = m_CurrentPayrollMailMergeData.EmployeeNumber.GetValueOrDefault(0)
					...
					Dim recipientAddress As String = String.Format("{0}", m_CurrentPayrollMailMergeData.EMail)
					...
					Else
						If m_MailSender <> recipientAddress Then
							Dim addContactData = AddEmployeeContactData(m_MailSender, recipientAddress, String.Empty, String.Empty, subject, m_HtmlConverter.ConvertToPlainText(m_RtfContent.HtmlText), attachments)
						End If
		...
		Private Function LoadMailSubjectData() As String
			...
			Select Case PreselectionData.MailType
				...
				Case MailTypeEnum.MOREPAYROLLS
					...
					subject = Regex.Replace(subject, REGEX_NumberOfPayrolls, String.Format("{0:F0}", m_NumberOfPayrolls))
		...
		Private Sub LoadmyMailTemplate(ByVal tplFilename As String)
			...
			Try
				If Not File.Exists(tplFilename) AndAlso tplFilename.ToLower.Contains("payroll") Then
...
#Region "employee contact table"
		...
		Private Function AddEmployeeContactData(ByVal strFrom As String, ByVal strTo As String,
			...
			Try
				...
				Select Case PreselectionData.MailType
					Case MailTypeEnum.MOREPAYROLLS
						title = String.Format("{0}", m_Translate.GetSafeTranslationValue("Versand von mehreren Lohnabrechnungen"))
				...
				result = result AndAlso m_CurrentPayrollMailMergeData.EmployeeNumber.GetValueOrDefault(0) > 0 AndAlso SaveEmployeeContactData(title, description, contactType, CType(Format(Now, "d"), Date), CType(Format(Now, "t"), DateTime), Nothing)
		...
		Private Function SaveEmployeeContactData(ByVal title As String, ByVal description As String, ByVal contactType As String, ByVal contactDate As Date, ByVal contactTime As DateTime, ByVal attachedFile As String) As Boolean
			...
			Dim contactData As SP.DatabaseAccess.Employee.DataObjects.ContactMng.EmployeeContactData = Nothing
			Dim fileContent = m_Utility.LoadFileBytes(attachedFile)

			Dim dt = DateTime.Now
			contactData = New SP.DatabaseAccess.Employee.DataObjects.ContactMng.EmployeeContactData With {.EmployeeNumber = m_EmployeeNumber, .CreatedOn = dt, .CreatedFrom = m_InitializationData.UserData.UserFullName}
...
#End Region
		...
		'				result = result AndAlso m_CurrentPayrollMailMergeData.EmployeeNumber.GetValueOrDefault(0) > 0 AndAlso SaveCustomerContactData(title, strTo, description, contactType, Nothing)
...
#Region "Helpers"
		...
		Function ParseTemplateFile(ByVal FullFileName As DevExpress.XtraRichEdit.RichEditControl) As String
			...
			Try
				...
				If Not m_CurrentPayrollMailMergeData Is Nothing Then

					myRegEx_0 = New Regex(REGEX_NumberOfPayrolls)
					result_0 = FullFileName.Document.StartSearch(myRegEx_0)
					While result_0.FindNext()
						result_0.Replace(m_CurrentPayrollMailMergeData.NumberOfPayrolls)
					End While

					myRegEx_0 = New Regex(REGEX_WhenMoreThanOne)
					result_0 = FullFileName.Document.StartSearch(myRegEx_0)
					Dim value As String = String.Empty
					If m_CurrentPayrollMailMergeData.NumberOfPayrolls > 1 Then value = "en"
					While result_0.FindNext()
...
#End If
		...
		'Private Function IsEMailAddressExists(ByVal toEmail As String) As Boolean
...
#End Region
		...
		Private Class PayrollEMailMergeViewData
			Inherits PayrollEMailMergeData
