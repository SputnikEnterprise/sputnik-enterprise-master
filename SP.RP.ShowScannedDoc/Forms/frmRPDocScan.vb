...
#Region "Constructor"
	...
	Public Sub New(ByVal _setting As SP.Infrastructure.Initialization.InitializeClass)
		...
		m_MandantData = New SPProgUtility.Mandanten.Mandant
		...
		m_RPDatabaseAccess = New ReportDatabaseAccess(connectionString, m_InitialData.UserData.UserLanguage)
		m_WOSDatabaseAccess = New WOSDatabaseAccess(connectionString, m_InitialData.UserData.UserLanguage)
		...
		Me.KeyPreview = True
		Dim strStyleName As String = m_MandantData.GetSelectedUILayoutName(m_InitialData.MDData.MDNr, m_InitialData.UserData.UserNr, String.Empty)
...
#End Region
	...
	Private Sub frmRPDocScan_FormClosing(sender As Object, e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
		...
		Try
			...
			Try
				...
				Else
					If bChkKDWOS.Visibility = DevExpress.XtraBars.BarItemVisibility.Always AndAlso bChkKDWOS.Enabled Then My.Settings.bSendRPToKDWOS = Me.bChkKDWOS.EditValue
					If bChkMAWOS.Visibility = DevExpress.XtraBars.BarItemVisibility.Always AndAlso bChkMAWOS.Enabled Then My.Settings.bSendRPToMAWOS = Me.bChkMAWOS.EditValue
	...
	Private Sub OnForm_Load(sender As Object, e As System.EventArgs) Handles Me.Load
		...
		Try
			If Not String.IsNullOrWhiteSpace(m_CustomerWOSID) Then
			...
			If Not String.IsNullOrWhiteSpace(m_EmployeeWOSID) Then
			...
			End If
			If Me.bChkKDWOS.Visibility = DevExpress.XtraBars.BarItemVisibility.Never AndAlso Me.bChkMAWOS.Visibility = DevExpress.XtraBars.BarItemVisibility.Never Then
	...
	Sub ShowFileInViewer(ByVal strFilename As String, ByVal pdfViewer As DevExpress.XtraPdfViewer.PdfViewer)
		...
		Try
			...
			pdfViewer.ZoomMode = DevExpress.XtraPdfViewer.PdfZoomMode.Custom
	...
	Private Function LoadScanedContentData() As Boolean
		...
		Me.bsiInfo.Caption = String.Format(m_Translate.GetSafeTranslationValue("Anzahl Datensätze: {0}"), gvScanContent.RowCount)
	...
	Private Function LoadCustomerCategoriesDropDownData(ByVal language As String) As Boolean
		Dim categoryData = m_CustomerDatabaseAccess.LoadCustomerDocumentCategoryData()
	...
	Private Function LoadReportsDropDownData() As Boolean
		...
		m_ReportOverviewData = m_RPDatabaseAccess.LoadRPOverviewListData()
	...
	Private Sub InsertRecordIntoMainTables()
		...
		For Each rec In selectedData
			...
			ElseIf m_ScanModulType = ScanTypes.reports Then
				...
				If success AndAlso (_ClsProgSetting.GetKDWOSGuid.Length > 10 AndAlso rec.SendToCustomerWOS) Then
					Dim rpScanData = reportDBPersister.LoadAssignedScannedReport(New DBInformation With {.SelectedRecordNumber = rec.SP_RecordNumber, .SelectedRPLNr = rec.SP_RPLNumber, .EmployeeNumber = rec.SP_EmployeeNumber, .CustomerNumber = rec.SP_CustomerNumber})

					success = success AndAlso TransferEmployeeReportDataToWOS(rec.SP_RecordNumber, rec.SP_RPLNumber, rpScanData.DocScan)
					success = success AndAlso TransferCustomerReportDataToWOS(rec.SP_RecordNumber, rec.SP_RPLNumber, rpScanData.DocScan)

					'Dim rpScanGuid = m_RPDatabaseAccess.GetRPLScanDocGuid(iOpenedRPNr, iOpenedRPLNr) ' LoadAssignedScannedReport(Data)

					'Dim newDocGuid As String = Guid.NewGuid.ToString

					'Dim objWosInfo As New ClsWOSInfomation With {.SelectedESLohnNr = 0,
					'																					 .SelectedESNr = rec.SP_EinsatzNumber,
					'																					 .SelectedKDNr = rec.SP_CustomerNumber,
					'																					 .SelectedMANr = rec.SP_EmployeeNumber,
					'																					 .SelectedRPNr = rec.SP_RecordNumber,
					'																					 .SelectedRPLNr = rec.SP_RPLNumber,
					'																					 .SelectedDocGuid = newDocGuid,
					'																					 .SendRPToKDWOS = rec.SendToCustomerWOS,
					'																					 .SendRPToMAWOS = rec.SendToEmployeeWOS}
	...
	Sub SaveIndividualReportIntoDatabase()
		...
		Try
			...
			Else
				strMessage = m_Translate.GetSafeTranslationValue("Ihre Daten konnten <b>nicht</b> gespeichert werden!")
			End If


			Dim transferResult As Boolean = True
			Dim rpScanGuid = m_RPDatabaseAccess.GetRPLScanDocGuid(iOpenedRPNr, iOpenedRPLNr)
			m_OpenedDocGuid = rpScanGuid

			If Not String.IsNullOrWhiteSpace(m_EmployeeWOSID) Then
				If Not bChkMAWOS.EditValue Then
					If Not String.IsNullOrWhiteSpace(rpScanGuid) Then transferResult = success AndAlso DeleteEmployeeReportDocumentFromWOS(iOpenedRPNr, rpScanGuid)

				Else
					transferResult = success AndAlso TransferEmployeeReportDataToWOS(docData.SelectedRecordNumber, docData.SelectedRPLNr, docData.DocScan)
				End If
			End If

			If Not String.IsNullOrWhiteSpace(m_CustomerWOSID) Then
				If Not bChkKDWOS.EditValue Then
					If Not String.IsNullOrWhiteSpace(rpScanGuid) Then transferResult = transferResult AndAlso DeleteCustomerReportDocumentFromWOS(iOpenedRPNr, rpScanGuid)

				Else
					transferResult = transferResult AndAlso TransferCustomerReportDataToWOS(docData.SelectedRecordNumber, docData.SelectedRPLNr, docData.DocScan)
				End If
			End If


			If Not transferResult Then
				strMessage &= m_Translate.GetSafeTranslationValue("<br>Ihre Anfrage konnte <b>nicht</b> erfolgreich übermittelt werden!")
				m_UtilityUI.ShowOKDialog(strMessage, m_Translate.GetSafeTranslationValue("WOS-Übermittlung"), MessageBoxIcon.Asterisk)

			Else

				strMessage &= m_Translate.GetSafeTranslationValue("<br>Ihre Anfrage wurden erfolgreich übermittelt.")
				m_UtilityUI.ShowOKDialog(strMessage, m_Translate.GetSafeTranslationValue("WOS-Übermittlung"), MessageBoxIcon.Information)
	...
	Private Function TransferEmployeeReportDataToWOS(ByVal reportNumber As Integer, ByVal reportLineNumber As Integer, ByVal file2Transfer As Byte()) As Boolean
		Dim result As Boolean = True
		If String.IsNullOrWhiteSpace(m_EmployeeWOSID) OrElse Not bChkMAWOS.EditValue Then Return True
		...
		If rpData Is Nothing OrElse rplineData Is Nothing Then
			...
			Return False
		End If
		' rpl is not in employee
		If rplineData.ID = 0 Then Return True
		...
		If Not employeeData.Send2WOS OrElse String.IsNullOrWhiteSpace(employeeData.Email) Then
			m_Logger.LogDebug(String.Format("Der Kandiddat {0} ist nicht WOS-pflichtig!", m_CurrentCustomerNumber))
			bChkMAWOS.Enabled = False
	...
	Function TransferCustomerReportDataToWOS(ByVal reportNumber As Integer, ByVal reportLineNumber As Integer, ByVal file2Transfer As Byte()) As Boolean
		Dim result As Boolean = True
		...
		If String.IsNullOrWhiteSpace(m_CustomerWOSID) OrElse Not bChkKDWOS.EditValue Then Return True

		Dim rpData As RPMasterData = m_RPDatabaseAccess.LoadRPMasterData(reportNumber)
		Dim rpScanDocGuid As String = m_RPDatabaseAccess.GetRPLScanDocGuid(reportNumber, reportLineNumber)
		Dim rplineData As RPLListData = m_WOSDatabaseAccess.LoadRPLineData(reportNumber, RPLType.Customer, reportLineNumber)
	...
	Private Function DeleteEmployeeReportDocumentFromWOS(ByVal rpNr As Integer, ByVal scanDocGuid As String) As Boolean
		...
		Dim rpData = m_RPDatabaseAccess.LoadRPMasterData(rpNr)
		Dim employeeData = m_EmployeeDatabaseAccess.LoadEmployeeMasterData(rpData.EmployeeNumber, False)
	...
	Private Function DeleteCustomerReportDocumentFromWOS(ByVal rpNr As Integer, ByVal scanDocGuid As String) As Boolean
		Dim result As Boolean = True
		Dim wos = New SP.Internal.Automations.WOSUtility.CustomerExport(m_InitialData)
		Dim wosSetting = New WOSSendSetting

		Dim rpData = m_RPDatabaseAccess.LoadRPMasterData(rpNr)
		Dim customerData = m_CustomerDatabaseAccess.LoadCustomerMasterData(rpData.CustomerNumber, "%%")
	...
	Sub DeleteOpenedScanFile()
		...
		Dim dbInfo As DBInformation = New DBInformation With {.EmployeeNumber = Me.iOpenedMANr, .CustomerNumber = Me.iOpenedKDNr, .ESNumber = Me.iOpenedESNr, .SelectedRecordNumber = Me.iOpenedRPNr, .SelectedRPLNr = Me.iOpenedRPLNr, .OriginFileGuid = m_OpenedDocGuid}
		If String.IsNullOrWhiteSpace(PdfViewer1.DocumentFilePath) Then Return
		...
		Dim employeeData = m_EmployeeDatabaseAccess.LoadEmployeeMasterData(iOpenedMANr, False)
		If employeeData Is Nothing Then Return
		dbInfo.EmployeeGuid = employeeData.Transfered_Guid

		Dim customerData = m_CustomerDatabaseAccess.LoadCustomerMasterData(iOpenedKDNr, "%%")
		If customerData Is Nothing Then Return
		dbInfo.CustomerGuid = customerData.Transfered_Guid

		Try
			success = success AndAlso Me.reportDBPersister.DeleteAssignedReport(dbInfo)
