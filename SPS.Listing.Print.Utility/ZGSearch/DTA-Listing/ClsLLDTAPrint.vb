...
Public Class ClsLLDTAPrint
	...
	Private m_PrintSetting As New ClsLLDTAPrintSetting
	...
	Private m_ConnString As String = String.Empty
...
#Region "Constructor"
	...
	Public Sub New(ByVal _Setting As ClsLLDTAPrintSetting)

		Me.m_PrintSetting = _Setting
		m_InitializationData = _Setting.m_initData
		m_Translate = _Setting.m_Translate

		Me.m_ConnString = m_InitializationData.MDData.MDDbConn
...
#End Region
	...
	Function BuildPrintJob() As Boolean
		...
		Dim Conn As SqlConnection = New SqlConnection(m_ConnString)
	...
	Sub ShowInDesign()
		...
		Try
			...
			If m_PrintSetting.JobNr2Print = "6.2.1" Then
				rFoundedrec = MainUtilities.LoadZGADataForLOLDTAList(m_ConnString, m_InitializationData.MDData.MDNr, Me.m_PrintSetting.dtaNumber)
			Else
				rFoundedrec = MainUtilities.LoadZGADataForDTAList(m_ConnString, m_InitializationData.MDData.MDNr, Me.m_PrintSetting.dtaNumber)
			End If
			...
			LL.Core.LlDefineLayout(CType(m_PrintSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel,
	...
	Function ShowInPrint(ByRef bShowBox As Boolean) As Boolean
		...
		Try
			...
			Else
				DefineData(LL, False, rFoundedrec)
			End If
			'LL.DataSource = m_PrintSetting.m_dtaDataList
			SetLLVariable(LL)
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper),
																		 ...
																		 CType(m_PrintSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel)
	...
	Sub SetLLVariable(ByVal LL As ListLabel)
		...
		For Each number In Me.m_PrintSetting.dtaNumber
			dtaNumbersBuffer = dtaNumbersBuffer & IIf(dtaNumbersBuffer <> "", ", ", "") & number
		Next
		LL.Variables.Add("dtaNumber", dtaNumbersBuffer)
	...
	Sub GetMDUSData4Print(ByVal LL As ListLabel)
		...
		MainUtilities.GetUSData(m_ConnString, _ClsLLFunc, m_InitializationData.MDData.MDNr)
		With _ClsLLFunc
		...
		'Dim MandantenData = LoadMandantDataForDTAList(m_ConnString, m_initData.MDData.MDNr, m_PrintSetting.MandantBankIDNumber)
		Dim MandantenData = m_ListingDatabaseAccess.LoadMandantDataForPrintDTAESRListData(m_InitializationData.MDData.MDNr, m_PrintSetting.MandantBankIDNumber)
		If Not MandantenData Is Nothing Then
			...
			LL.Variables.Add("DTAIBAN", MandantenData.DTAIBAN)
			LL.Variables.Add("KontoVG", MandantenData.KontoVG)
			LL.Variables.Add("VGIBAN", MandantenData.VGIBAN)

			LL.Variables.Add("dtafiledate", Me.m_PrintSetting.DTAfiledate)
			LL.Variables.Add("DTAFileName", Me.m_PrintSetting.DTAFileName)
		End If
