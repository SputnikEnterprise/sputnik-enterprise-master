...
Imports SPS.Listing.Print.Utility.AdvancePaymentDbDatabases.ClsAdvancedPaymentDb4Print
...
Public Class ClsLLAdvancePaymentPrint
	...
	Private printSetting As New ClsLLAdvancePaymentPrintSetting
	...
	Private m_ZGData As ZGMasterData
...
#Region "Contructor"
	...
	Public Sub New(ByVal _Setting As ClsLLAdvancePaymentPrintSetting)
		...
		Me.m_connectionString = printSetting.DbConnString2Open

		m_ZGDatabaseAccess = New AdvancePaymentDatabaseAccess(m_connectionString, m_InitialData.UserData.UserLanguage)

		Dim recordMandantNumber As Integer = m_InitialData.MDData.MDNr
		LoadZGData(printSetting.ZGNr)
		If Not m_ZGData Is Nothing Then
			recordMandantNumber = m_ZGData.MDNr
		End If
		If Not (recordMandantNumber = m_InitialData.MDData.MDNr OrElse m_InitialData.MDData.MultiMD = 0) Then
			printSetting.SelectedMDNr = recordMandantNumber
		End If
...
#End Region
	...
	Function BuildPrintJob() As Boolean
		...
		If JobNr = String.Empty Then
			...
			MsgBox(String.Format((strMessage), vbNewLine),
		...
		Try
			...
			If Not String.IsNullOrEmpty(rDocrec("DocName").ToString) Then
				_ClsLLFunc.LLDocName = m_md.GetSelectedMDDocPath(Me.printSetting.SelectedMDNr) & rDocrec("DocName").ToString
	...
	Sub ShowInDesign()
		...
		Try
			...
			rFoundedrec = LoadDb4PrintAdvancePaymentDocument(printSetting)
	...
	Function ShowInPrint(ByRef bShowBox As Boolean) As String
		...
		Try
			...
			If bShowBox Then
				LL.Core.LlPrintOptionsDialog(CType(printSetting.frmhwnd, IntPtr), String.Format("{1}: {2}{0}{3}", vbNewLine, _ClsLLFunc.LLDocLabel, strJobNr, _ClsLLFunc.LLDocName))
			End If
			...
			If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, m_path.GetSpSTempFolder, printSetting.frmhwnd)
				LL.Core.LlPreviewDeleteFiles(_ClsLLFunc.LLDocName, m_path.GetSpSTempFolder)
				Return "Error"
			End If
			UpdateAdvancePaymentPrintDate(printSetting)
	...
	Function ExportLLDoc(ByVal LL As ListLabel, ByVal strJobNr As String, ByVal iExportMode As Short,
		...
		Try
			...
			If IsNothing(rFoundedrec) Then
				...
				MsgBox((strMessage), MsgBoxStyle.Critical, ("Daten suchen"))
				Return bResult
			End If
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), LlProject.List, LlProject.Card),
																	_ClsLLFunc.LLDocName, LlPrintMode.Export, LlBoxType.StandardAbort, CType(printSetting.frmhwnd, IntPtr),
			...
			If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, m_path.GetSpSTempFolder, CType(printSetting.frmhwnd, IntPtr))
				LL.Core.LlPreviewDeleteFiles(_ClsLLFunc.LLDocName, m_path.GetSpSTempFolder)
			End If


		Catch LlException As Exception
			m_Logger.LogDebug(String.Format("LlException.Message:{0}", LlException.ToString))
	...
	Sub GetMDUSData4Print(ByVal LL As ListLabel)
		...
		LoadUserPrintData(Me.printSetting.DbConnString2Open, _ClsLLFunc, printSetting.ZGNr)
	...
	Private Function LoadZGData(ByVal zgNumber As Integer) As Boolean
		Dim result As Boolean = True

		m_ZGData = m_ZGDatabaseAccess.LoadZGMasterData(zgNumber)

		If m_ZGData Is Nothing Then
			m_UtilityUI.ShowErrorDialog(m_Translate.GetSafeTranslationValue("Vorschuss-Daten für Mandant-Spezifikationen konnten nicht geladen werden."))
		End If


		Return (Not m_ZGData Is Nothing)
	...
	Sub SetLLVariable(ByVal LL As ListLabel)
		...
		LL.Variables.Add("ListBez2Print", printSetting.ListBez2Print)
