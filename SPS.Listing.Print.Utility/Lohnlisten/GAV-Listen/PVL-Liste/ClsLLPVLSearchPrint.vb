...
Public Class ClsLLPVLSearchPrint
  ...
  Private m_md As New Mandant
	Private PVLListPrintSetting As New ClsLLPVLSearchPrintSetting
  ...
  Function BuildPrintJob() As Boolean
    ...
    Dim JobNr As String = PVLListPrintSetting.JobNr2Print
    ...
    Try
      ...
      param = cmd.Parameters.AddWithValue("@JobNr", PVLListPrintSetting.JobNr2Print)
      ...
      If Not String.IsNullOrEmpty(rDocrec("DocName").ToString) Then
        _ClsLLFunc.LLDocName = m_md.GetSelectedMDDocPath(Me.PVLListPrintSetting.SelectedMDNr) & rDocrec("DocName").ToString
  ...
  Sub ShowInDesign()
    ...
    Try
      Dim rFoundedrec As SqlDataReader = MainUtilities.OpenDb4PrintListing(strConnString, Me.PVLListPrintSetting.SQL2Open)
      ...
      SetLLVariable(LL)
			...
			LL.Core.LlDefineLayout(CType(PVLListPrintSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel, _
  ...
  Function ShowInPrint(ByRef bShowBox As Boolean) As String
    ...
    Dim strMethodeName As String = New System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name
		...
		Dim strJobNr As String = Me.PVLListPrintSetting.JobNr2Print
    Try
    ...
    Dim strQuery As String = PVLListPrintSetting.SQL2Open
    Dim Conn As SqlConnection = New SqlConnection(strConnString)
    Dim i As Integer = 0
    ...
    Try
      Dim rFoundedrec As SqlDataReader = Utilities.OpenDb4PrintListing(strConnString, PVLListPrintSetting.SQL2Open)
      ...
      SetLLVariable(LL)
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), _
																		 ...
																		 CType(PVLListPrintSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel)
      ...
      If bShowBox Then
				LL.Core.LlPrintOptionsDialog(CType(PVLListPrintSetting.frmhwnd, IntPtr), String.Format("{1}: {2}{0}{3}", _
																																	 vbNewLine, _
      ...
      If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 m_path.GetSpSTempFolder, CType(PVLListPrintSetting.frmhwnd, IntPtr))
  ...
  Function ExportLLDoc(ByVal LL As ListLabel, ByVal strJobNr As String, ByVal iExportMode As Short, _
    ...
    Try
      ...
      SetExportSetting(iExportMode)
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), LlProject.List, LlProject.Card), _
																	_ClsLLFunc.LLDocName, LlPrintMode.Export, LlBoxType.StandardAbort, CType(PVLListPrintSetting.frmhwnd, IntPtr), _
      ...
      If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 ...
																 CType(PVLListPrintSetting.frmhwnd, IntPtr))
    ...
    Catch LlException As Exception
			If Err.Number <> (LlError.LL_ERR_USER_ABORTED) And Err.Number <> 5 Then
				m_Logger.LogError(LlException.ToString)
			End If
  ...
  End Sub
    ...
    LL.Variables.Add("USSignFilename", Me.PVLListPrintSetting.USSignFileName)

    LL.Variables.Add("SortBez", PVLListPrintSetting.ListSortBez)

		LL.Variables.Add("FilterBez", PVLListPrintSetting.ListFilterBez(0))
		LL.Variables.Add("FilterBez2", If(PVLListPrintSetting.ListFilterBez.Count > 1, PVLListPrintSetting.ListFilterBez(1), String.Empty))
		LL.Variables.Add("FilterBez3", If(PVLListPrintSetting.ListFilterBez.Count > 2, PVLListPrintSetting.ListFilterBez(2), String.Empty))
		LL.Variables.Add("FilterBez4", If(PVLListPrintSetting.ListFilterBez.Count > 3, PVLListPrintSetting.ListFilterBez(3), String.Empty))

    LL.Variables.Add("ListBez2Print", PVLListPrintSetting.ListBez2Print)
    LL.Variables.Add("Vorjahreslohnsumme", PVLListPrintSetting.Jahreslohnsumme)
		LL.Variables.Add("LLAGPauschale", PVLListPrintSetting.AGPauschale)
		LL.Variables.Add("PauschaleChecked", PVLListPrintSetting.PauschaleChecked.GetValueOrDefault(False))

		' Mandantendaten drucken...
		GetMDUSData4Print(LL)

		If PVLListPrintSetting.SearchAddressDataFromTableseting.GetValueOrDefault(False) Then
			LoadGAVAddressData()
		End If
	...
	Sub GetMDUSData4Print(ByVal LL As ListLabel)
		...
		MainUtilities.GetUSData(Me.PVLListPrintSetting.DbConnString2Open, _ClsLLFunc, PVLListPrintSetting.SelectedMDNr)
		With _ClsLLFunc
	...
	Private Sub LoadGAVAddressData()

		Dim conn As SqlConnection = New SqlConnection(_ClsProgSetting.GetConnString)
		Try

			' Die anzuzeigende Spalten von der Tabelle holen
			Dim sql As String = String.Format("SELECT Top 1 * FROM TAB_InkassoPoolPrint WHERE GAVBeruf = '{0}'", PVLListPrintSetting.GAVBezeichnung)
			...
			reader.Read()
			For i As Integer = 1 To reader.FieldCount - 1 ' Das erste Feld ist die GAVBeruf-Bezeichnung und muss nicht geladen werden.
				LL.Variables.Add(reader.GetName(i), CBool(reader.GetValue(i).ToString))
			Next
    ...
    Me.PVLListPrintSetting = _Setting
