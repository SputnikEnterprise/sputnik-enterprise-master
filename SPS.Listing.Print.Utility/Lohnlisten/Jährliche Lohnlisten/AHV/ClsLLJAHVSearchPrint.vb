...
Public Class ClsLLJAHVSearchPrint
	...
	Private JLohnRekapSetting As New ClsLLJAHVSearchPrintSetting
...
#Region "Private Consts"
	Private Const FORM_XML_MAIN_KEY As String = "MD_{0}"
#End Region
	...
	Function BuildPrintJob() As Boolean
		...
		Dim JobNr As String = JLohnRekapSetting.JobNr2Print
		...
		Try
			...
			param = cmd.Parameters.AddWithValue("@JobNr", JLohnRekapSetting.JobNr2Print)
			...
			If Not String.IsNullOrEmpty(rDocrec("DocName").ToString) Then
				_ClsLLFunc.LLDocName = m_md.GetSelectedMDDocPath(Me.JLohnRekapSetting.SelectedMDNr) & rDocrec("DocName").ToString
	...
	Sub ShowInDesign()
		...
		Try
			Dim rFoundedrec As SqlDataReader = MainUtilities.OpenDb4PrintListing(strConnString, Me.JLohnRekapSetting.SQL2Open)
			...
			LL.Core.LlDefineLayout(CType(JLohnRekapSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel, _
	...
	Function ShowInPrint(ByRef bShowBox As Boolean) As String
		...
		Dim strJobNr As String = Me.JLohnRekapSetting.JobNr2Print
		Try
		...
		Dim strQuery As String = JLohnRekapSetting.SQL2Open
		Dim Conn As SqlConnection = New SqlConnection(strConnString)
		Dim i As Integer = 0
		...
		Try
			Dim rFoundedrec As SqlDataReader = Utilities.OpenDb4PrintListing(strConnString, JLohnRekapSetting.SQL2Open)
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), _
																		 ...
																		 CType(JLohnRekapSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel)
			...
			If bShowBox Then
				LL.Core.LlPrintOptionsDialog(CType(JLohnRekapSetting.frmhwnd, IntPtr), String.Format("{1}: {2}{0}{3}", _
																																	 vbNewLine, _
			...
			If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 m_path.GetSpSTempFolder, CType(JLohnRekapSetting.frmhwnd, IntPtr))
	...
	Function ExportLLDoc(ByVal LL As ListLabel, ByVal strJobNr As String, ByVal iExportMode As Short, _
		...
		Try
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), LlProject.List, LlProject.Card), _
																	_ClsLLFunc.LLDocName, LlPrintMode.Export, LlBoxType.StandardAbort, CType(JLohnRekapSetting.frmhwnd, IntPtr), _
			...
			If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 ...
																 CType(JLohnRekapSetting.frmhwnd, IntPtr))
	...
	Sub GetMDUSData4Print(ByVal LL As ListLabel)
		...
		MainUtilities.GetUSData(Me.JLohnRekapSetting.DbConnString2Open, _ClsLLFunc, JLohnRekapSetting.SelectedMDNr)
		With _ClsLLFunc
	...
	Sub SetLLVariable(ByVal LL As ListLabel)
		...
		Dim FORM_XML_MAIN_KEY As String = String.Format("MD_{0}", JLohnRekapSetting.SelectedMDNr)
		Dim mdXMLFile As String = m_md.GetSelectedMDDataXMLFilename(JLohnRekapSetting.SelectedMDNr, Now.Year)
		...
		LL.Variables.Add("USSignFilename", Me.JLohnRekapSetting.USSignFileName)

		LL.Variables.Add("FilterBez", JLohnRekapSetting.ListFilterBez(0))
		LL.Variables.Add("FilterBez2", If(JLohnRekapSetting.ListFilterBez.Count > 1, JLohnRekapSetting.ListFilterBez(1), String.Empty))
		LL.Variables.Add("FilterBez3", If(JLohnRekapSetting.ListFilterBez.Count > 2, JLohnRekapSetting.ListFilterBez(2), String.Empty))
		LL.Variables.Add("FilterBez4", If(JLohnRekapSetting.ListFilterBez.Count > 3, JLohnRekapSetting.ListFilterBez(3), String.Empty))

		LL.Variables.Add("ListBez2Print", JLohnRekapSetting.ListBez2Print)

		Dim AusgNummer As String = m_path.GetXMLNodeValue(mdXMLFile, String.Format("{0}/AHV-Daten/AusgNummer", FORM_XML_MAIN_KEY))
		...
		LL.Variables.Add("AusgleichsKassennummer", AusgNummer)
		LL.Variables.Add("SuvaKundennummer", Abrechnungsnummer)
