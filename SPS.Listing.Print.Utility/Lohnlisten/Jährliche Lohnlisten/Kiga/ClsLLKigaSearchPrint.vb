...
Public Class ClsLLKigaSearchPrint
	...
	Private KigaSetting As New ClsLLKigaSearchPrintSetting
	...
	Function BuildPrintJob() As Boolean
		...
		Dim JobNr As String = KigaSetting.JobNr2Print
		...
		Try
			...
			param = cmd.Parameters.AddWithValue("@JobNr", KigaSetting.JobNr2Print)
			...
			If Not String.IsNullOrEmpty(rDocrec("DocName").ToString) Then
				_ClsLLFunc.LLDocName = m_md.GetSelectedMDDocPath(Me.KigaSetting.SelectedMDNr) & rDocrec("DocName").ToString
	...
	Sub ShowInDesign()
		...
		Try
			Dim rFoundedrec As SqlDataReader = MainUtilities.OpenDb4PrintListing(strConnString, Me.KigaSetting.SQL2Open)
			...
			End If
			SetLLVariable(LL, rFoundedrec)

			LL.Core.LlDefineLayout(CType(KigaSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel, _
	...
	Function ShowInPrint(ByRef bShowBox As Boolean) As String
		...
		Dim strJobNr As String = Me.KigaSetting.JobNr2Print
		Try
		...
		Dim strQuery As String = KigaSetting.SQL2Open
		Dim Conn As SqlConnection = New SqlConnection(strConnString)
		Dim i As Integer = 0
		...
		Try
			Dim rFoundedrec As SqlDataReader = Utilities.OpenDb4PrintListing(strConnString, KigaSetting.SQL2Open)
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), _
																		 ...
																		 CType(KigaSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel)
			...
			If bShowBox Then
				LL.Core.LlPrintOptionsDialog(CType(KigaSetting.frmhwnd, IntPtr), String.Format("{1}: {2}{0}{3}", _
																																	 vbNewLine, _
			...
			If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 m_path.GetSpSTempFolder, CType(KigaSetting.frmhwnd, IntPtr))
	...
	Function ExportLLDoc(ByVal LL As ListLabel, ByVal strJobNr As String, ByVal iExportMode As Short, _
		...
		Try
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), LlProject.List, LlProject.Card), _
																	_ClsLLFunc.LLDocName, LlPrintMode.Export, LlBoxType.StandardAbort, CType(KigaSetting.frmhwnd, IntPtr), _
			...
			If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 ...
																 CType(KigaSetting.frmhwnd, IntPtr))
		...
		Catch LlException As Exception
			m_Logger.LogDebug(String.Format("LlException.Message:{0}This information was generated by a List & Label custom exception.", vbNewLine))
	...
	Sub GetMDUSData4Print(ByVal LL As ListLabel)
		...
		MainUtilities.GetUSData(Me.KigaSetting.DbConnString2Open, _ClsLLFunc, KigaSetting.SelectedMDNr)
		With _ClsLLFunc
	...
	Sub SetLLVariable(ByVal LL As ListLabel, ByVal reader As SqlClient.SqlDataReader)
		...
		LL.Variables.Add("USSignFilename", Me.KigaSetting.USSignFileName)

		LL.Variables.Add("FilterBez", KigaSetting.ListFilterBez(0))
		LL.Variables.Add("FilterBez2", If(KigaSetting.ListFilterBez.Count > 1, KigaSetting.ListFilterBez(1), String.Empty))
		LL.Variables.Add("FilterBez3", If(KigaSetting.ListFilterBez.Count > 2, KigaSetting.ListFilterBez(2), String.Empty))
		LL.Variables.Add("FilterBez4", If(KigaSetting.ListFilterBez.Count > 3, KigaSetting.ListFilterBez(3), String.Empty))

		LL.Variables.Add("ListBez2Print", KigaSetting.ListBez2Print)

		LL.Variables.Add("TempYear", m_Utility.SafeGetInteger(reader, "Jahr", Now.Year))
		LL.Variables.Add("TotalSMänner", m_Utility.SafeGetDecimal(reader, "TotalSMaenner", Nothing))
		LL.Variables.Add("TotalSFrauen", m_Utility.SafeGetDecimal(reader, "TotalSFrauen", Nothing))
		LL.Variables.Add("TotalAMänner", m_Utility.SafeGetDecimal(reader, "TotalAMaenner", Nothing))
		LL.Variables.Add("TotalAFrauen", m_Utility.SafeGetDecimal(reader, "TotalAFrauen", Nothing))
		LL.Variables.Add("AnzSMänner", m_Utility.SafeGetDecimal(reader, "AnzSMaenner", Nothing))
		LL.Variables.Add("AnzSFrauen", m_Utility.SafeGetDecimal(reader, "AnzSFrauen", Nothing))
		LL.Variables.Add("AnzAMänner", m_Utility.SafeGetDecimal(reader, "AnzAMaenner", Nothing))
		LL.Variables.Add("AnzAFrauen", m_Utility.SafeGetDecimal(reader, "AnzAFrauen", Nothing))
	...
	Public Sub New(ByVal _Setting As ClsLLKigaSearchPrintSetting)

		Me.KigaSetting = _Setting
