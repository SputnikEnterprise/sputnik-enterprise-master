...
Public Class ClsLLUVGSearchPrint
  ...
  Private m_md As New Mandant
	Private UVGListPrintSetting As New ClsLLUVGSearchPrintSetting
  ...
  Function BuildPrintJob() As Boolean
    ...
    Dim JobNr As String = UVGListPrintSetting.JobNr2Print
    ...
    Try
      ...
      param = cmd.Parameters.AddWithValue("@JobNr", UVGListPrintSetting.JobNr2Print)
      ...
      If Not String.IsNullOrEmpty(rDocrec("DocName").ToString) Then
        _ClsLLFunc.LLDocName = m_md.GetSelectedMDDocPath(Me.UVGListPrintSetting.SelectedMDNr) & rDocrec("DocName").ToString
  ...
  Sub ShowInDesign()
    ...
    Try
      Dim rFoundedrec As SqlDataReader = MainUtilities.OpenDb4PrintListing(strConnString, Me.UVGListPrintSetting.SQL2Open)
      ...
      SetLLVariable(LL)
			...
			LL.Core.LlDefineLayout(CType(UVGListPrintSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel, _
  ...
  Function ShowInPrint(ByRef bShowBox As Boolean) As String
    ...
    Dim strMethodeName As String = New System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name
		...
		Dim strJobNr As String = Me.UVGListPrintSetting.JobNr2Print
    Try
    ...
    Dim strQuery As String = UVGListPrintSetting.SQL2Open
    Dim Conn As SqlConnection = New SqlConnection(strConnString)
    Dim i As Integer = 0

    If Not Me.ExistsDocFile Then Return "Error: No File could be find..."

    Try
      Dim rFoundedrec As SqlDataReader = Utilities.OpenDb4PrintListing(strConnString, UVGListPrintSetting.SQL2Open)
      ...
      SetLLVariable(LL)
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), _
																		 ...
																		 CType(UVGListPrintSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel)
      ...
      If bShowBox Then
				LL.Core.LlPrintOptionsDialog(CType(UVGListPrintSetting.frmhwnd, IntPtr), String.Format("{1}: {2}{0}{3}", _
																																	 vbNewLine, _
      ...
      If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 m_path.GetSpSTempFolder, CType(UVGListPrintSetting.frmhwnd, IntPtr))
    ...
    Try
      ...
      SetExportSetting(iExportMode)
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), LlProject.List, LlProject.Card), _
																	_ClsLLFunc.LLDocName, LlPrintMode.Export, LlBoxType.StandardAbort, CType(UVGListPrintSetting.frmhwnd, IntPtr), _
      ...
      If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 ...
																 CType(UVGListPrintSetting.frmhwnd, IntPtr))
  ...
  Sub GetMDUSData4Print(ByVal LL As ListLabel)
		...
		MainUtilities.GetUSData(Me.UVGListPrintSetting.DbConnString2Open, _ClsLLFunc, UVGListPrintSetting.SelectedMDNr)
		With _ClsLLFunc
  ...
  Sub SetLLVariable(ByVal LL As ListLabel)
    ...
    LL.Variables.Add("USSignFilename", Me.UVGListPrintSetting.USSignFileName)

    LL.Variables.Add("SortBez", UVGListPrintSetting.ListSortBez)

		LL.Variables.Add("FilterBez", UVGListPrintSetting.ListFilterBez(0))
		LL.Variables.Add("FilterBez2", If(UVGListPrintSetting.ListFilterBez.Count > 1, UVGListPrintSetting.ListFilterBez(1), String.Empty))
		LL.Variables.Add("FilterBez3", If(UVGListPrintSetting.ListFilterBez.Count > 2, UVGListPrintSetting.ListFilterBez(2), String.Empty))
		LL.Variables.Add("FilterBez4", If(UVGListPrintSetting.ListFilterBez.Count > 3, UVGListPrintSetting.ListFilterBez(3), String.Empty))

    LL.Variables.Add("ListBez2Print", UVGListPrintSetting.ListBez2Print)

    ' Die selektierte Zeit-Periode
    LL.Variables.Add("SelectedJahrVon", UVGListPrintSetting.JahrVon2Print)
    LL.Variables.Add("SelectedJahrBis", UVGListPrintSetting.JahrBis2Print)
    LL.Variables.Add("SelectedMonatVon", UVGListPrintSetting.MonatVon2Print)
    LL.Variables.Add("SelectedMonatBis", UVGListPrintSetting.MonatBis2Print)

    ' Totale MAxx, FAxx, MZxx und FZxx hinzufügen
    LL.Variables.Add("TotalMA12", UVGListPrintSetting.TotalSuva.Item(0)) 'LLTotalMA12)
    LL.Variables.Add("TotalMA3", UVGListPrintSetting.TotalSuva.Item(1)) 'LLTotalMA3)
    LL.Variables.Add("TotalFA12", UVGListPrintSetting.TotalSuva.Item(2)) 'LLTotalFA12)
    LL.Variables.Add("TotalFA3", UVGListPrintSetting.TotalSuva.Item(3)) 'LLTotalFA3)

    LL.Variables.Add("TotalMZ12", UVGListPrintSetting.TotalSuva.Item(4)) 'LLTotalMZ12)
    LL.Variables.Add("TotalMZ3", UVGListPrintSetting.TotalSuva.Item(5)) 'LLTotalMZ3)
    LL.Variables.Add("TotalFZ12", UVGListPrintSetting.TotalSuva.Item(6)) 'LLTotalFZ12)
    LL.Variables.Add("TotalFZ3", UVGListPrintSetting.TotalSuva.Item(7)) 'LLTotalFZ3)


    For i As Integer = 0 To UVGListPrintSetting.SecSuvaCode.Count - 1
      Dim strBez As String = UVGListPrintSetting.SecSuvaCode.Keys(i)
      Dim aValue As String() = UVGListPrintSetting.SecSuvaCode.Values(i).Split("|")

      LL.Variables.Add(String.Format("SecSuvaBez{0}", If(strBez.Trim.Length > 0, strBez, "0")), String.Empty)
      LL.Variables.Add(String.Format("SecSuvaM{0}", If(strBez.Trim.Length > 0, strBez, "0")), aValue(0), LlFieldType.Numeric_Localized)
      LL.Variables.Add(String.Format("SecSuvaF{0}", If(strBez.Trim.Length > 0, strBez, "0")), aValue(1), LlFieldType.Numeric_Localized)

    Next

    LL.Variables.Add("ESAnzahlM", UVGListPrintSetting.esAnzM)
    LL.Variables.Add("ESAnzahlF", UVGListPrintSetting.esAnzF)
    LL.Variables.Add("ShowESAnzahl", UVGListPrintSetting.LLShowESAnzJahr)
    LL.Variables.Add("ESAnzahlJahr", UVGListPrintSetting.LLESAnzJahr)
    LL.Variables.Add("SUVA_HL", UVGListPrintSetting.MDsuva_hl)
    LL.Variables.Add("UnfallNr", UVGListPrintSetting.ausgleichsNr)
  ...
  Public Sub New(ByVal _Setting As ClsLLUVGSearchPrintSetting)
    ...
    Me.UVGListPrintSetting = _Setting
