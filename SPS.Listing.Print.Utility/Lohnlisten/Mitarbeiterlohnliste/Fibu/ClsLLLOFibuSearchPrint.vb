...
Public Class ClsLLLOFibuSearchPrint
  ...
  Private m_md As New Mandant
	Private _LOFibuListPrintSetting As New ClsLLLOFibuSearchPrintSetting
  ...
  Sub ShowInDesign()
    ...
    Try
      Dim rFoundedrec As SqlDataReader = MainUtilities.OpenDb4PrintListing(strConnString, Me._LOFibuListPrintSetting.SQL2Open)
  ...
  End Sub
	...
	Function ShowInPrint(ByRef bShowBox As Boolean) As PrintResult
		Const LL_PRNOPT_COPIES_SUPPORTED As Integer = 3
		Dim strJobNr As String = Me._LOFibuListPrintSetting.JobNr2Print
		...
		If Not Me.ExistsDocFile Then Return New PrintResult With {.Printresult = False,
			.PrintresultMessage = String.Format("{0}: Vorlage wurde nicht gefunden.", _LOFibuListPrintSetting.JobNr2Print)}
		...
		Dim strQuery As String = _LOFibuListPrintSetting.SQL2Open
		Dim Conn As SqlConnection = New SqlConnection(strConnString)
		Dim i As Integer = 0


		Try
			Dim rFoundedrec As SqlDataReader = Utilities.OpenDb4PrintListing(strConnString, _LOFibuListPrintSetting.SQL2Open)
			If (rFoundedrec Is Nothing) OrElse Not rFoundedrec.HasRows Then Return New PrintResult With {.Printresult = False, .PrintresultMessage = String.Format("Keine Daten wurden gefunden.")}
  ...
  Sub GetMDUSData4Print(ByVal LL As ListLabel)
		...
		MainUtilities.GetUSData(Me._LOFibuListPrintSetting.DbConnString2Open, _ClsLLFunc, _LOFibuListPrintSetting.SelectedMDNr)
		With _ClsLLFunc
  ...
  Sub SetLLVariable(ByVal LL As ListLabel)
    ...
    LL.Variables.Add("USSignFilename", Me._LOFibuListPrintSetting.USSignFileName)

    LL.Variables.Add("SortBez", _LOFibuListPrintSetting.ListSortBez)
    LL.Variables.Add("FilterBez", _LOFibuListPrintSetting.ListFilterBez(0))
    LL.Variables.Add("FilterBez2", _LOFibuListPrintSetting.ListFilterBez(1))
    LL.Variables.Add("FilterBez3", _LOFibuListPrintSetting.ListFilterBez(2))
    LL.Variables.Add("FilterBez4", _LOFibuListPrintSetting.ListFilterBez(3))

    LL.Variables.Add("ListBez2Print", _LOFibuListPrintSetting.ListBez2Print)
  ...
  Public Sub New(ByVal _Setting As ClsLLLOFibuSearchPrintSetting)
    ...
    Me._LOFibuListPrintSetting = _Setting
    Me.strConnString = _Setting.DbConnString2Open
    Me.ExistsDocFile = MainUtilities.BuildPrintJob(_Setting.SelectedMDNr, _Setting.DbConnString2Open, _Setting.JobNr2Print, _ClsLLFunc)
