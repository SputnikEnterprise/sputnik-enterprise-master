...
Public Class ClsLLLOStdSearchPrint
	...
	Private LOStdListPrintSetting As New ClsLLLOStdSearchPrintSetting
	...
	Function BuildPrintJob() As Boolean
		...
		Dim JobNr As String = LOStdListPrintSetting.JobNr2Print
		...
		Try
			...
			param = cmd.Parameters.AddWithValue("@JobNr", LOStdListPrintSetting.JobNr2Print)
			...
			If Not String.IsNullOrEmpty(rDocrec("DocName").ToString) Then
				_ClsLLFunc.LLDocName = m_md.GetSelectedMDDocPath(Me.LOStdListPrintSetting.SelectedMDNr) & rDocrec("DocName").ToString
	...
	Sub ShowInDesign()
		...
		Try
			Dim rFoundedrec As SqlDataReader = MainUtilities.OpenDb4PrintListing(strConnString, Me.LOStdListPrintSetting.SQL2Open)
	...
	Function ShowInPrint(ByRef bShowBox As Boolean) As String
		...
		Dim strJobNr As String = Me.LOStdListPrintSetting.JobNr2Print

		Dim strQuery As String = LOStdListPrintSetting.SQL2Open
		Dim Conn As SqlConnection = New SqlConnection(strConnString)
		Dim i As Integer = 0
		...
		Try
			Dim rFoundedrec As SqlDataReader = Utilities.OpenDb4PrintListing(strConnString, LOStdListPrintSetting.SQL2Open)
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), _
																		 ...
																		 CType(LOStdListPrintSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel)
			...
			If bShowBox Then
				LL.Core.LlPrintOptionsDialog(CType(LOStdListPrintSetting.frmhwnd, IntPtr), String.Format("{1}: {2}{0}{3}", _
																																	 vbNewLine, _
			...
			If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 m_path.GetSpSTempFolder, CType(LOStdListPrintSetting.frmhwnd, IntPtr))
				LL.Core.LlPreviewDeleteFiles(_ClsLLFunc.LLDocName, m_path.GetSpSTempFolder)
				Return "Error"
			End If


		Catch ex As LL_User_Aborted_Exception

		Catch ex As Exception
			strResult = String.Format("Error: {0}", ex.ToString)
			m_Logger.LogWarning(String.Format("{0}: {1}", strMethodeName, ex.ToString))
	...
	Function ExportLLDoc(ByVal LL As ListLabel, ByVal strJobNr As String, ByVal iExportMode As Short, _
		...
		Try
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), LlProject.List, LlProject.Card), _
																	_ClsLLFunc.LLDocName, LlPrintMode.Export, LlBoxType.StandardAbort, CType(LOStdListPrintSetting.frmhwnd, IntPtr), _
			...
			If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 ...
																 CType(LOStdListPrintSetting.frmhwnd, IntPtr))
	...
	Sub GetMDUSData4Print(ByVal LL As ListLabel)
		...
		MainUtilities.GetUSData(Me.LOStdListPrintSetting.DbConnString2Open, _ClsLLFunc, LOStdListPrintSetting.SelectedMDNr)
		With _ClsLLFunc
	...
	Sub SetLLVariable(ByVal LL As ListLabel)
		...
		LL.Variables.Add("USSignFilename", Me.LOStdListPrintSetting.USSignFileName)

		LL.Variables.Add("SortBez", LOStdListPrintSetting.ListSortBez)
		LL.Variables.Add("FilterBez", LOStdListPrintSetting.ListFilterBez(0), LlFieldType.Text_Allow_Wordwrap)
		LL.Variables.Add("FilterBez2", If(LOStdListPrintSetting.ListFilterBez.Count > 1, LOStdListPrintSetting.ListFilterBez(1), String.Empty))
		LL.Variables.Add("FilterBez3", If(LOStdListPrintSetting.ListFilterBez.Count > 2, LOStdListPrintSetting.ListFilterBez(2), String.Empty))
		LL.Variables.Add("FilterBez4", If(LOStdListPrintSetting.ListFilterBez.Count > 3, LOStdListPrintSetting.ListFilterBez(3), String.Empty))

		LL.Variables.Add("ListBez2Print", LOStdListPrintSetting.ListBez2Print)
	...
	Public Sub New(ByVal _Setting As ClsLLLOStdSearchPrintSetting)

		Me.LOStdListPrintSetting = _Setting
