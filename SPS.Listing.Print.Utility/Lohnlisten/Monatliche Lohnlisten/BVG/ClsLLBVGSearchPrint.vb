...
Public Class ClsLLBVGSearchPrint
...
#End Region
	...
	Private _Setting As New ClsLLBVGSearchPrintSetting
	...
	Function BuildPrintJob() As Boolean
		...
		Dim sSql As String = "Select D.*, MD.BVG_List, MD.BVG_List_Grouped From DokPrint D, Mandanten MD Where D.JobNr = @JobNr And MD.MDNr = @MDNr And MD.Jahr = @Year"
		Dim Conn As SqlConnection = New SqlConnection(strConnString)
		Dim bResult As Boolean = True
		Dim JobNr As String = _Setting.JobNr2Print
		...
		Try
			Conn.Open()
			Dim mdYear As Integer = _Setting.SelectedMDYear
			If mdYear = 0 Then mdYear = Now.Year

			param = cmd.Parameters.AddWithValue("@JobNr", _Setting.JobNr2Print)
			param = cmd.Parameters.AddWithValue("@MDNr", _Setting.SelectedMDNr)
			param = cmd.Parameters.AddWithValue("@Year", mdYear)
			Dim rDocrec As SqlDataReader = cmd.ExecuteReader
			rDocrec.Read()
			If Not String.IsNullOrEmpty(rDocrec("DocName").ToString) Then
				Dim docName As String = If(_Setting.JobNr2Print.Contains(".G"), rDocrec("BVG_List_Grouped").ToString, rDocrec("BVG_List").ToString)
				If docName = String.Empty OrElse Not File.Exists(String.Format("{0}{1}", m_md.GetSelectedMDDocPath(Me._Setting.SelectedMDNr), docName)) Then docName = rDocrec("DocName").ToString
				_ClsLLFunc.LLDocName = Path.Combine(m_md.GetSelectedMDDocPath(Me._Setting.SelectedMDNr), docName) ' String.Format("{0}{1}", m_md.GetSelectedMDDocPath(Me._Setting.SelectedMDNr), docName)

				If Not File.Exists(_ClsLLFunc.LLDocName) Then
					Dim msg As String = "Die gesuchte Vorlage wurde nicht gefunden.{0}{1}: {2}"
					msg = String.Format(TranslateMyText(msg), vbNewLine, _Setting.JobNr2Print, _ClsLLFunc.LLDocName)
	...
	Sub ShowInDesign()
		...
		Try
			...
			Dim bAllowedtoDebug As Boolean = CBool(m_path.GetXMLValueByQueryWithFilename(m_md.GetSelectedMDDataXMLFilename(ClsMainSetting.MDData.MDNr, Now.Year),
		...
		Try
			Dim rFoundedrec As SqlDataReader = MainUtilities.OpenDb4PrintBVGListing(strConnString, _Setting.SQL2Open)
			...
			If _ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper) Then
				result = result AndAlso DefineData(True, rFoundedrec)
			Else
				result = result AndAlso DefineData(False, rFoundedrec)
			End If
			result = result AndAlso SetLLVariable()

			If Not result Then Return
			LL.Core.LlDefineLayout(CType(_Setting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel, If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), LlProject.List, LlProject.Card), _ClsLLFunc.LLDocName)
	...
	Function ShowInPrint(ByRef bShowBox As Boolean) As String
		...
		Dim strMethodeName As String = New System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name
		Dim strJobNr As String = Me._Setting.JobNr2Print

		If Not Me.ExistsDocFile Then Return "File not found!"

		Dim strQuery As String = _Setting.SQL2Open
		Dim Conn As SqlConnection = New SqlConnection(strConnString)
		Dim i As Integer = 0
		...
		Try
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), _
																		 ...
																		 CType(_Setting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel)
			...
			If bShowBox Then
				LL.Core.LlPrintOptionsDialog(CType(_Setting.frmhwnd, IntPtr), String.Format("{1}: {2}{0}{3}", _
																																	 vbNewLine, _
			...
			If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 m_path.GetSpSTempFolder, CType(_Setting.frmhwnd, IntPtr))
	...
	Private Function DefineData(ByVal AsFields As Boolean, ByVal MyDataReader As SqlDataReader) As Boolean
		Dim result As Boolean = True
		Dim i As Integer
		Dim iType As String
	...
	Private Function GetMDUSData4Print() As Boolean
		Dim result As Boolean = True

		Try
			MainUtilities.GetUSData(Me._Setting.DbConnString2Open, _ClsLLFunc, _Setting.SelectedMDNr)
			With _ClsLLFunc
			...
			MainUtilities.GetMandantData(Me._Setting.DbConnString2Open, Me._Setting.SelectedMDYear, _ClsLLFunc)
			With _ClsLLFunc
				LL.Variables.Add("BVG_Koordination_Jahr", .bvgkoordinationlohnjahr)
				LL.Variables.Add("BVG_Koordination_Std", .bvgkoordinationlohnstd)
				LL.Variables.Add("BVG_Max_Jahr", .bvgmaximallohnjahr)
				LL.Variables.Add("BVG_Min_Jahr", .bvgminmallohnjahr)
				LL.Variables.Add("BVG_Std", .bvgstd)
				LL.Variables.Add("BVG_Woche", .bvgwoche)
	...
	Private Function SetLLVariable() As Boolean
		Dim result As Boolean = True
		...
		Try
			...
			LL.Variables.Add("USSignFilename", Me._Setting.USSignFileName)

			LL.Variables.Add("SortBez", _Setting.ListSortBez)
			LL.Variables.Add("FilterBez", _Setting.ListFilterBez(0))
			LL.Variables.Add("FilterBez2", If(_Setting.ListFilterBez.Count > 1, _Setting.ListFilterBez(1), String.Empty))
			LL.Variables.Add("FilterBez3", If(_Setting.ListFilterBez.Count > 2, _Setting.ListFilterBez(2), String.Empty))
			LL.Variables.Add("FilterBez4", If(_Setting.ListFilterBez.Count > 3, _Setting.ListFilterBez(3), String.Empty))
			LL.Fields.Add("bLineinDiffColor", _ClsLLFunc.LLPrintInDiffColor)

			LL.Variables.Add("SelektionText", _Setting.LLSelektionText)

			LL.Variables.Add("SelektionText", _Setting.LLSelektionText)
			Dim bvgagnr As String = String.Empty
			Dim bvgname As String = String.Empty

			If Not m_MandantSettingsXml Is Nothing Then
				bvgname = m_MandantSettingsXml.GetSettingByKey(String.Format("{0}/bvgname", m_SonstigesSetting))
				bvgagnr = m_MandantSettingsXml.GetSettingByKey(String.Format("{0}/bvgagnr", m_SonstigesSetting))
			End If

			LL.Variables.Add("bvgname", bvgname)
			LL.Variables.Add("bvgagnr", bvgagnr)
	...
	Public Sub New(ByVal _Setting As ClsLLBVGSearchPrintSetting)
		...
		m_MandantXMLFile = m_mandant.GetSelectedMDDataXMLFilename(_Setting.SelectedMDNr, _Setting.SelectedMDYear)
		...
		End If
		m_SonstigesSetting = String.Format(MANDANT_XML_SETTING_SPUTNIK_SONSTIGES_SETTING, _Setting.SelectedMDNr)
