...
Public Class ClsLLKDZHDSearchPrint
...
#End Region
  ...
  Private m_md As New Mandant
	Private ZHDListPrintSetting As New ClsLLKDZHDSearchPrintSetting
  ...
  Function BuildPrintJob() As Boolean
    ...
    Dim JobNr As String = ZHDListPrintSetting.JobNr2Print
    ...
    If JobNr = String.Empty Then
      Dim strMessage As String = "Sie haben keine Vorlage ausgewählt.{0}Bitte wählen Sie aus der Liste eine Vorlage aus."
		...
		Try
			...
			param = cmd.Parameters.AddWithValue("@JobNr", ZHDListPrintSetting.JobNr2Print)
			...
			If Not String.IsNullOrEmpty(rDocrec("DocName").ToString) Then
				_ClsLLFunc.LLDocName = m_md.GetSelectedMDDocPath(Me.ZHDListPrintSetting.SelectedMDNr) & rDocrec("DocName").ToString
	...
	Sub ShowInDesign()
		...
		Try
			...
			' _ClsProgSetting.GetMDProfilValue("Sonstiges", "EnableLLDebug", "0"))
		...
		Try
			Dim rFoundedrec As SqlDataReader = MainUtilities.OpenDb4PrintKDZHDListing(strConnString, Me.ZHDListPrintSetting.CustomerNumber, Me.ZHDListPrintSetting.CReocordNumbers)
	...
	Function ShowInPrint(ByRef bShowBox As Boolean) As String
		...
		Dim strMethodeName As String = New System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name
		Dim strJobNr As String = Me.ZHDListPrintSetting.JobNr2Print

		Dim strQuery As String = ZHDListPrintSetting.SQL2Open
		Dim Conn As SqlConnection = New SqlConnection(strConnString)
		Dim i As Integer = 0
  ...
  Sub GetMDUSData4Print()
		...
		MainUtilities.GetUSData(Me.ZHDListPrintSetting.DbConnString2Open, _ClsLLFunc, ZHDListPrintSetting.SelectedMDNr)
		With _ClsLLFunc
  ...
  Sub SetLLVariable()
    ...
    LL.Variables.Add("USSignFilename", Me.ZHDListPrintSetting.USSignFileName)

    LL.Variables.Add("SortBez", ZHDListPrintSetting.ListSortBez)
    LL.Variables.Add("FilterBez", ZHDListPrintSetting.ListFilterBez(0))
    LL.Variables.Add("FilterBez2", If(ZHDListPrintSetting.ListFilterBez.Count > 1, ZHDListPrintSetting.ListFilterBez(1), String.Empty))
    LL.Variables.Add("FilterBez3", If(ZHDListPrintSetting.ListFilterBez.Count > 2, ZHDListPrintSetting.ListFilterBez(2), String.Empty))
    LL.Variables.Add("FilterBez4", If(ZHDListPrintSetting.ListFilterBez.Count > 3, ZHDListPrintSetting.ListFilterBez(3), String.Empty))
  ...
  Public Sub New(ByVal _Setting As ClsLLKDZHDSearchPrintSetting)
    ...
    Me.ZHDListPrintSetting = _Setting
