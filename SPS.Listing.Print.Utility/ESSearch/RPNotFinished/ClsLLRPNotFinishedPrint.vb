...
Public Class ClsLLRPNotFinishedPrint
  ...
  Dim _RPListPrintSetting As New ClsLLRPNotFinishedPrintSetting
  ...
  Function BuildPrintJob() As Boolean
    ...
    Dim JobNr As String = _RPListPrintSetting.JobNr2Print
    ...
    If JobNr = String.Empty Then
      Dim strMessage As String = "Sie haben für JobNr {1} keine Vorlage ausgewählt.{0}Bitte wählen Sie aus der Liste eine Vorlage aus."
			...
			MsgBox(String.Format(TranslateMyText(strMessage), vbNewLine, JobNr), _
		...
		Try
			...
			param = cmd.Parameters.AddWithValue("@JobNr", _RPListPrintSetting.JobNr2Print)
			...
			If Not String.IsNullOrEmpty(rDocrec("DocName").ToString) Then
				_ClsLLFunc.LLDocName = m_md.GetSelectedMDDocPath(Me._RPListPrintSetting.SelectedMDNr) & rDocrec("DocName").ToString
	...
	Sub ShowInDesign()
		...
		Try
			Dim rFoundedrec As SqlDataReader = OpenDb4PrintListing(strConnString, _RPListPrintSetting.SQL2Open)
			...
			LL.Core.LlDefineLayout(CType(_RPListPrintSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel, _
	...
	Function ShowInPrint(ByRef bShowBox As Boolean) As String
		...
		Dim strJobNr As String = Me._RPListPrintSetting.JobNr2Print
		Try
		...
		Dim strQuery As String = _RPListPrintSetting.SQL2Open
		Dim Conn As SqlConnection = New SqlConnection(strConnString)
		Dim i As Integer = 0
		...
		Try
			Dim rFoundedrec As SqlDataReader = MainUtilities.OpenDb4PrintListing(strConnString, Me._RPListPrintSetting.SQL2Open)
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), _
																		 ...
																		 CType(_RPListPrintSetting.frmhwnd, IntPtr), _ClsLLFunc.LLDocLabel)
			...
			If bShowBox Then
				LL.Core.LlPrintOptionsDialog(CType(_RPListPrintSetting.frmhwnd, IntPtr), String.Format("{1}: {2}{0}{3}", _
																																	 vbNewLine, _
			...
			If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 m_path.GetSpSTempFolder, CType(_RPListPrintSetting.frmhwnd, IntPtr))
	...
	Function ExportLLDoc(ByVal strJobNr As String, ByVal iExportMode As Short, _
		...
		Try
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), LlProject.List, LlProject.Card), _
																	_ClsLLFunc.LLDocName, LlPrintMode.Export, LlBoxType.StandardAbort, CType(_RPListPrintSetting.frmhwnd, IntPtr), _
			...
			If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, _
																 ...
																 CType(_RPListPrintSetting.frmhwnd, IntPtr))
  ...
  Sub GetMDUSData4Print()
		...
		MainUtilities.GetUSData(Me._RPListPrintSetting.DbConnString2Open, _ClsLLFunc, _RPListPrintSetting.SelectedMDNr)
		With _ClsLLFunc
  ...
  Sub SetLLVariable()
    ...
    LL.Variables.Add("USSignFilename", Me._RPListPrintSetting.USSignFileName)

    LL.Variables.Add("SortBez", _RPListPrintSetting.ListSortBez)
    LL.Variables.Add("FilterBez", _RPListPrintSetting.ListFilterBez(0))
    LL.Variables.Add("FilterBez2", _RPListPrintSetting.ListFilterBez(1))
    LL.Variables.Add("FilterBez3", _RPListPrintSetting.ListFilterBez(2))
    LL.Variables.Add("FilterBez4", _RPListPrintSetting.ListFilterBez(3))
  ...
  Public Sub New(ByVal _Setting As ClsLLRPNotFinishedPrintSetting)
    ...
    Me._RPListPrintSetting = _Setting
