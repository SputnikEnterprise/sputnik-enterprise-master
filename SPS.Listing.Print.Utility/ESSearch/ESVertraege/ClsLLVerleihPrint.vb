...
Public Class ClsLLVerleihPrint
...
#Region "private properties"
	...
	Private ReadOnly Property GetESEndeByNull() As String
		Get
			Dim esendebynull As String = m_ProgPath.GetXMLNodeValue(m_MandantFormXMLFile, String.Format("{0}/esendebynullvv", FORM_XML_DEFAULTVALUES_KEY))
			If String.IsNullOrWhiteSpace(esendebynull) Then esendebynull = "unbestimmt"
...
#Region "Constructor"
	...
	Public Sub New(ByVal _setting As ClsLLESVertragSetting)
		...
		m_MandantSetting = String.Format(MANDANT_XML_MAIN_KEY, _setting.SelectedMDNr)
...
#End Region
		...
		Try
			...
			If Not String.IsNullOrEmpty(rDocrec("DocName").ToString) Then
				_ClsLLFunc.LLDocName = String.Format("{0}{1}{2}",
																						 m_mandant.GetSelectedMDDocPath(Me._ESVertragSetting.SelectedMDNr),
	...
	Function ExportLLDoc() As Boolean
		...
		Try
			...
			LL.Core.LlPrintWithBoxStart(If(_ClsLLFunc.LLDocName.ToUpper.EndsWith(".LST".ToUpper), LlProject.List, LlProject.Card),
																	_ClsLLFunc.LLDocName, LlPrintMode.Export, LlBoxType.None, CType(0, IntPtr),
																	_ClsLLFunc.LLDocLabel)
			...
			'Dim strExportFilename As String = Me._ESVertragSetting.GetExportFilename(True)
			...
			Try
				LL.Core.LlPrintEnd(0)
			Catch ex As Exception
				m_Logger.LogError(String.Format("LlPrintEnd...: {0}", ex.ToString))
				Return False
			End Try

			_ESVertragSetting.ListOfExportedFilesVerleih.Add(Path.Combine(strExportPfad, strExportFilename))
			If TargetFormat = "PRV" Then
				LL.Core.LlPreviewDisplay(_ClsLLFunc.LLDocName, m_ProgPath.GetSpSTempFolder, CType(0, IntPtr))
				LL.Core.LlPreviewDeleteFiles(_ClsLLFunc.LLDocName, m_ProgPath.GetSpSTempFolder)
			...
			Catch ex As Exception
				m_Logger.LogError(String.Format("LL Disposing...:{0}", ex.ToString))
		...
		Catch LlException As Exception
			m_Logger.LogError(String.Format("LlException. ESNr: {0} | MANr: {1}: {2}",
																 Me._ESVertragSetting.SelectedESNr2Print,
																 Me._ESVertragSetting.SelectedMANr2Print,
																 LlException.Message))
			strResult = False
	...
	Sub GetMDUSData4Print()
		...
		LoadUserPrintData(_ESVertragSetting.DbConnString2Open, _ClsLLFunc, _ESVertragSetting.SelectedESNr2Print)
		Try
			With _ClsLLFunc
				...
				LL.Variables.Add("MwStProzent", mwstsatz) ' _ClsReg.GetINIString(_ClsProgSetting.GetMDIniFile, "Debitoren", "MWST-Satz"))
	...
	Private Sub LoadUserPrintData(ByVal strConnString As String, ByVal _ClsLLFunc As ClsLLFunc, ByVal recordNumber As Integer)
		Dim strMethodeName As String = New System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name
		...
		If Not m_ESData Is Nothing Then
			recordMandantNumber = m_ESData.MDNr
		End If
		'If recordMandantNumber = m_InitialData.MDData.MDNr OrElse m_InitialData.MDData.MultiMD = 0 Then recordMandantNumber = 0
	...
	Sub SetLLVariable(ByVal rFrec As SqlDataReader)
		...
		With rFrec
			Try
				Dim iKDZustaendigNr As Integer? = m_Utility.SafeGetInteger(rFrec, "KDZHDNr", Nothing)
				...
				Dim KDzAnrede_Prog As String = MainUtilities.GetColumnTextStr(rFrec, "KDzAnrede", "")
				...
				Dim customerLanguage = m_Utility.SafeGetString(rFrec, "KDSprache")
				...
				If m_Utility.SafeGetString(rFrec, "ES_Ende") = String.Empty Then
					...
					strES_EndeDate = GetSafeTranslationValue(strES_EndeDate, customerLanguage.Substring(0, 1).ToUpper)
					isESEndeNull = True
				...
				LL.Variables.Add("KDzAnrede_Prog", GetSafeTranslationValue(KDzAnrede_Prog, _ESVertragSetting.SelectedLang))
		...
		Try
			...
			Dim rFoundedrec As SqlDataReader = OpenMDDb4PrintESVertrag(_ESVertragSetting, Year(dESLoVon))
			If Not rFoundedrec.HasRows Then
	...
	Sub GetUSData4ESVertrag()
		...
		'Dim strFiles As String = String.Empty
		'Dim BA As Byte() = Nothing
		'Dim bSignFromUser As Boolean = If(Me._ESVertragSetting.GetESUnterzeichner = 1, True, False)
		'Dim strAllESKst As String = String.Empty

		'Dim sUSSql As String = String.Empty
		''sUSSql = "Declare @ESUS_Anrede nvarchar(4) "
		''sUSSql &= "Set @ESUS_Anrede = (Select IsNull((Select Top 1 Anrede From Benutzer "
		''sUSSql &= "Where Vorname + ' ' + Nachname = (Select ESUnterzeichner From ES Where ESNr = @ESNr) ), '')) "

		''sUSSql &= "Declare @US_Anrede nvarchar(4) "
		''sUSSql &= "Set @US_Anrede = (Select IsNull((Select Top 1 Anrede From Benutzer Where USNr = @USNr), '')) "

		''sUSSql &= "Declare @AllESKst nvarchar(50) "
		''sUSSql &= "Set @AllESKst = (Select IsNull((Select Top 1 ESKst From ES Where ESNr = @ESNr), '')) "

		''sUSSql &= "Select @ESUS_Anrede As ESUS_Anrede, @US_Anrede As US_Anrede, @AllESKst As AllESKst From Benutzer Where "

		''If bSignFromUser Then
		''  sUSSql &= "USNr = @USNr "

		''Else
		''  sUSSql &= "Vorname + ' ' + Nachname = "
		''  sUSSql &= "IsNull((Select ESUnterzeichner From ES Where ESNr = @ESNr), '') "
		...
		''  Dim aKst As String() = strAllESKst.Split(CChar("/"))
		''  If aKst.Length = 1 Then aKst = New String() {aKst(0), aKst(0)}

		''  sUSSql = "Select Top 1 IsNull(Anrede, '') As Anrede, IsNull(Vorname, '') As Vorname, IsNull(Nachname, '') As Nachname, IsNull(USTitel_1, '') As USTitel_1, "
		''  sUSSql &= "IsNull(USTitel_2, '') As USTitel_2, IsNull(Abteilung, '') As USAbteilung From "
		''  sUSSql &= "Benutzer Where KST = @Kst1"
		''  cmd = New SqlCommand(sUSSql, Conn)
		''  param = New System.Data.SqlClient.SqlParameter

		''  If Me._ESVertragSetting.IsPrintAsVerleih Then
		''    param = cmd.Parameters.AddWithValue("@Kst1", Trim(aKst(1)))
		''  Else
		''    param = cmd.Parameters.AddWithValue("@Kst1", Trim(aKst(0)))
		''  End If
		''  rTemprec = cmd.ExecuteReader
	...
	Sub Test_GetUSSign4ESVertrag()
		...
		Dim strFiles As String = String.Empty
		Dim BA As Byte() = Nothing
		Dim bSignFromUser As Boolean = m_ESUnterzeichner ' Me._ESVertragSetting.GetESUnterzeichner = 1
		...
		'sUSSql &= "Where Vorname + ' ' + Nachname = (Select ESUnterzeichner From ES Where ESNr = @ESNr) ), '')) "
		'sUSSql &= "Declare @US_Anrede nvarchar(4) "
		'sUSSql &= "Set @US_Anrede = (Select IsNull((Select Top 1 Anrede From Benutzer "
		'sUSSql &= "Where USNr = @USNr), '')) "
		'sUSSql &= "Declare @AllESKst nvarchar(50) "
		'sUSSql &= "Set @AllESKst = (Select IsNull((Select Top 1 ESKst From ES "
		'sUSSql &= "Where ESNr = @ESNr), '')) "
		'sUSSql &= "Select USSign From Benutzer Where  "
